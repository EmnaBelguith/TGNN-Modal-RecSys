...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def __init__(self, rating_values, review_embedding, user_size, item_size, msg_units, num_layers, aggregate='sum', dropout_rate=0.0):
        ...
        self.modal_gate = nn.Linear(msg_units * 2, msg_units)
        self.h_v = None
        self.h_t = None
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        for l in range(len(self.conv_layers)):
            ...
            for rating in self.rating_values:
                ...
                if l == 0: 
                    ...
                    self._last_item_ids = block['user', rating, 'item'].dstdata['_ID']
        ...
        # Gated fusion paper style : h_v et h_t fusionnés ici seulement
        ids = self._last_item_ids
        h_v_i = self.h_v[ids]
        h_t_i = self.h_t[ids]
        gi = torch.sigmoid(self.modal_gate(torch.cat([h_v_i, h_t_i], dim=-1)))
        zi = gi * h_v_i + (1 - gi) * h_t_i

        return user_outputs, item_outputs, zi
...
class Net(nn.Module):
    ...
    def predict_rating(self, input_nodes, encoder_blocks, decoder_graph):
        user_feat, item_feat, _ = self.rating_encoder(input_nodes, encoder_blocks)

        # zi pour les items du decoder_graph
        dec_item_ids = decoder_graph.nodes['item'].data['_ID']
        h_v_dec = self.rating_encoder.h_v[dec_item_ids]
        h_t_dec = self.rating_encoder.h_t[dec_item_ids]
        gi = torch.sigmoid(self.rating_encoder.modal_gate(
            torch.cat([h_v_dec, h_t_dec], dim=-1)))
        zi = gi * h_v_dec + (1 - gi) * h_t_dec

        predicts = self.topic_decoder.predict_rating(decoder_graph, user_feat, zi, None, None)
    ...
    def calc_loss(self, \
        ...
        urf, irf, _ = self.rating_encoder(rating_input_nodes, rating_encoder_blocks)
        utf, itf = self.topic_encoder(topic_input_nodes, topic_encoder_blocks)

        # zi calculé pour les items du decoder_graph (même taille que itf)
        dec_item_ids = decoder_graph.nodes['item'].data['_ID']
        h_v_dec = self.rating_encoder.h_v[dec_item_ids]
        h_t_dec = self.rating_encoder.h_t[dec_item_ids]
        gi = torch.sigmoid(self.rating_encoder.modal_gate(
        ...
        return rating_loss, ed_mi, ranking_loss, urf, irf, zi
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        ...
        for rating_input_nodes, decoder_graph, rating_encoder_blocks in dataloader:
            ...
            urf, irf, zi = self.rating_encoder(rating_input_nodes, rating_encoder_blocks)
...
def train(params):
   ...
   # === MODAL ENCODER ===
    ...
    item_degrees = torch.load('/home/infres/belguith/PFE/bm3_data/musical/item_degrees.pt', weights_only=False)
...
# =====================
    ...
    net.rating_encoder.h_v = h_v.to(params.device)
    net.rating_encoder.h_t = h_t.to(params.device)
    net.rating_encoder.item_degree = item_degrees.to(params.device)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, edge_subgraph, rating_blocks in pbar:
            ...
            net.rating_encoder.item_degree = item_degrees.to(params.device)
            r_loss, mi_score, ranking_loss, urf, irf, zi = net.calc_loss(
            ...
            # stats zi pour voir si le modal contribue
            with torch.no_grad():
                zi_norm = zi.norm(dim=-1).mean()
                urf_norm = urf.norm(dim=-1).mean()
                cosine = torch.nn.functional.cosine_similarity(
                    urf[:zi.shape[0]], zi, dim=-1).mean()

            pbar.set_description(
                f"loss={r_loss:.4f} modal={modal_loss.item():.4f} "
                f"|zi|={zi_norm:.3f} |urf|={urf_norm:.3f} cos(u,zi)={cosine:.3f}"
            )
        ...
        with torch.no_grad():
            ...
            _gate_input = torch.cat([net.rating_encoder.item_embedding, net.rating_encoder.h_modal[:_n]], dim=-1)
            _alpha = torch.sigmoid(net.rating_encoder.modal_gate(_gate_input))
            std_inter_items = _alpha.std(dim=0).mean()   # std par dim, moyennée
            std_inter_dims  = _alpha.std(dim=1).mean()   # std par item, moyennée
            print(f"[GATE] epoch={iter_idx} mean={_alpha.mean():.3f} std_global={_alpha.std():.3f} "
                  f"std_inter_items={std_inter_items:.3f} std_inter_dims={std_inter_dims:.3f} "
                  f"min={_alpha.min():.3f} max={_alpha.max():.3f}", flush=True)

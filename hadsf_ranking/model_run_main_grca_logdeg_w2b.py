...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_w2b{_run_tag}.pt'
    )
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        _log_deg_norm = (torch.log1p(_degrees_out.float()) /
...
class Net(nn.Module):
    ...
    def compute_fusion_loss(self, user_emb, h_modal_pos, h_modal_neg, lambda_f=1e-4, weights=None):
        pos_score = (user_emb * h_modal_pos).sum(dim=-1)
        neg_score = (user_emb * h_modal_neg).sum(dim=-1)
        per_item = -F.logsigmoid(pos_score - neg_score)          # (N,)
...
def train(params):
    ...
    with torch.no_grad():
        ...
        net.rating_encoder.register_buffer('_h_modal_target_norm', _target_norm.clone())
    _h_norm_orig = h_modal[:_n_items].norm(dim=-1).mean().item()
    print(f"[INIT] item_embedding ← h_modal direction (norm {_h_norm_orig:.3f} → {_target_norm:.3f})", flush=True)
    print(f"[INIT] h_modal normalisé à {_target_norm:.3f} à chaque epoch (LayerNorm structurel)", flush=True)
    ...
    with torch.no_grad():
        ...
        print(f"[GATE_INIT] _pre_ci_global_max={net.rating_encoder._pre_ci_global_max.item():.4f}  gate_T=softplus(2.0)={F.softplus(net.rating_encoder.gate_temp).item():.4f}", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            if params.no_modal:
                ...
                h_modal, h_v, h_t = modal_enc()
            h_modal = F.normalize(h_modal, dim=-1) * net.rating_encoder._h_modal_target_norm
            net.rating_encoder.h_modal = h_modal
            ...
            if not params.no_modal and _n_f > 1:
                ...
                # pondérer f_loss par alpha(item) — warm (α élevé) = signal BPR fiable, cold (α bas) = bruit
            ...
            if (not params.no_modal and params.lambda_grca > 0
                ...
                _w    = ((1 - net.rating_encoder._grca_alpha).detach().squeeze(-1)) ** 2  # (N,) — (1-α)² : relâche GRCA sur warm

...
# =====================
    ...
    _abl_disable_init = params.no_modal or params.no_image or params.no_text
    with torch.no_grad():
        if _abl_disable_init:
            # h_modal=0 quand pas de features → Xavier random init
            nn.init.xavier_uniform_(net.rating_encoder.item_embedding)
            print(f"[INIT] item_embedding ← Xavier (baseline sans modal)", flush=True)
        else:
            # Full model : direction h_modal, norme ramenée à Xavier (~0.12)
            _target_norm = net.rating_encoder.user_embedding.norm(dim=-1).mean()
            _h_dir = h_modal[:net.rating_encoder.item_embedding.shape[0]]
            _h_dir = _h_dir / _h_dir.norm(dim=-1, keepdim=True).clamp(min=1e-8)
            net.rating_encoder.item_embedding.data.copy_(_h_dir * _target_norm)
            print(f"[INIT] item_embedding ← h_modal direction (norm→{_target_norm:.3f})", flush=True)
    import pandas as pd

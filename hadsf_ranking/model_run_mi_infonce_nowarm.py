...
def config():
    ...
    parser.add_argument('--mi_temperature',    type=float, default=0.1,
                        help='Température InfoNCE pour la MI loss')
    ...
    def forward(self, graph, urf, irf, utf, itf, neg_strategy='random', n_neg=1,
        ...
        with graph.local_scope():
            if neg_strategy == 'inbatch':
                ...
                pos_review = self.get_review_feature(pos_sid)
                N = th.shape[0]
                # InfoNCE symétrique — scale-invariant (F.normalize neutralise la croissance GCN)
                th_n   = F.normalize(th, dim=-1)
                rev_n  = F.normalize(pos_review, dim=-1)
                T      = getattr(self, '_mi_temperature', 0.1)
                logits = th_n @ rev_n.T / T          # (N, N)
                labels = torch.arange(N, device=th.device)
                mi = (F.cross_entropy(logits, labels) + F.cross_entropy(logits.T, labels)) / 2
                return mi, mi
            else:
...
class Net(nn.Module):
    ...
    def __init__(self, review_embedding, sentence_embedding, params):
        ...
        self.topic_decoder._mi_temperature = params.mi_temperature
...
def train(params):
    ...
    topic_frozen  = False  # geler topic_encoder + topic_linear quand MI se stabilise
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            _eff_lambda_mi = (params.lambda_mi if iter_idx > params.lambda_mi_warmup else 0.0) if not topic_frozen else 0.0
        ...
        # ── Geler topic_encoder + topic_linear quand MI se stabilise ──
        if not topic_frozen and params.lambda_mi > 0 and _epoch_mi_count > 0:
            ...
            # freeze si MI n'a pas bougé de plus de 0.05 sur les 10 derniers epochs
            if len(_mi_history) >= 10:
                _mi_drop = _mi_history[-10] - _mi_history[-1]
                if abs(_mi_drop) < 0.05:
                    for param in net.topic_encoder.parameters():
                        param.requires_grad = False
                    for param in net.topic_decoder.topic_linear.parameters():
                        param.requires_grad = False
                    topic_frozen = True
                    print(f"[TOPIC_FREEZE] epoch={iter_idx}  avg_MI={_epoch_avg_mi:.4f}  "
                          f"drop/10ep={_mi_drop:.4f}  "
                          f"→ topic_encoder + topic_linear gelés (SR stabilisé)", flush=True)

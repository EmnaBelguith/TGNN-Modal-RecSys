...
def config():
    ...
    # ── Config fixe dataset ─────────────────────────────────────────────────
    args.dataset_name = 'Musical_HADSF_coldstart'
            ...
            for local_i, global_i in enumerate(g_iids):
                if global_i not in item_emb:
                    if self.rating_encoder.item_degree_tensor[global_i].item() == 0:
                        # Cold item : 0 voisins train → GCN output nul/bruit → bypass GCN, utilise h_modal
...
def train(params):
    ...
    _df_deg_train = pd.read_csv('/home/infres/belguith/PFE/processed/Musical_interactions_reviews_coldstart.csv')
    ...
    print("[INIT] item_degree_tensor injected (Musical_interactions_reviews_coldstart.csv — train only)", flush=True)

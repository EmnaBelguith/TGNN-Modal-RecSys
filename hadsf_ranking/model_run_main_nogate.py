...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        if self.training:
            _cc = item_outputs.norm(dim=-1).detach().cpu()
            _mc = _item_modal_out.norm(dim=-1).detach().cpu()
            self._epoch_mc_sum    += _mc.sum().item()
            self._epoch_cc_sum    += _cc.sum().item()
            self._epoch_contrib_n += _cc.shape[0]

        _adapted     = self.modal_adapter(torch.cat([item_outputs, _item_modal_out], dim=-1))
        item_outputs = item_outputs + _adapted  # résiduel pur, pas de gate
...
def train(params):
    ...
    with torch.no_grad():
        ...
        print(f"[NOGATE] résiduel pur — pas de gate", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        with torch.no_grad():
            ...
            if _n_c > 0:
                ...
                print(f"[CONTRIB] collab_norm={_cc:.3f} modal_norm={_mc:.3f}", flush=True)

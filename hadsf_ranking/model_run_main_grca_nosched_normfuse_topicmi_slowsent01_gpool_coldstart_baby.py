...
def config():
    ...
    # ── Config fixe dataset ─────────────────────────────────────────────────
    args.dataset_name = 'Baby_HADSF_coldstart'
            ...
            for local_i, global_i in enumerate(g_iids):
                if global_i not in item_emb:
                    if self.rating_encoder.item_degree_tensor[global_i].item() == 0:
                        item_emb[global_i] = self.rating_encoder.h_modal[global_i].detach().cpu()
                    else:
...
def train(params):
    ...
    _max_sid_pkl   = max(_sid_to_topic.keys())
    _max_sid_graph = int(torch.max(all_sentence_ids).item())
    _max_sid = max(_max_sid_pkl, _max_sid_graph)
    ...
    import pandas as pd
    # Baby_interactions_reviews.csv : IDs alignés sur Baby_reviews_item2id.json (0..5491)
    _df_deg_train = pd.read_csv('/home/infres/belguith/PFE/processed/Baby_interactions_reviews_coldstart.csv')
    ...
    print("[INIT] item_degree_tensor injected (Baby_interactions_reviews_coldstart.csv — train only)", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            # ── GRCA cold items — item_embedding comme proxy collab (pas de voisins GCN) ──
            # Pour deg=0 : item_outputs GCN = 0 (fn.sum sur ∅), donc non présents dans
            # _grca_item_collab. On utilise item_embedding (initialisé depuis h_modal direction)
            # comme cible collab. alpha=0 → w=1, même température que le GRCA warm.
            ...
            if not params.no_modal and params.lambda_grca > 0 and len(_diag_cold_idx) > 0:
                ...
                _col_cold     = net.rating_encoder.item_embedding[_cold_dev]   # (N_cold, d)
...
def test(params):
    import evaluate_model_run_coldstart as _em

...
def config():
    ...
    parser.add_argument('--no_image',          action='store_true', default=False,
                        help='Zéro les features image → h_modal = h_t (texte seul, pas de cross-attention)')
    parser.add_argument('--no_text',           action='store_true', default=False,
                        help='Zéro les features texte → h_modal = h_v (image seule, pas de cross-attention)')
    ...
    _nimg_tag  = '_noimage'              if args.no_image                  else ''
    _ntxt_tag  = '_notext'               if args.no_text                   else ''
    args.model_save_path = (
        f'model_save/{args.dataset_name}/{args.model_short_name}'
        f'_layers_{args.num_layers}_seed{args.seed}'
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}{_rinit_tag}{_bprw_tag}{_nimg_tag}{_ntxt_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_grca_emb{_run_tag}.pt'
    )
...
def train(params):
    ...
    if params.no_image:
        ...
        h_modal = h_t.clone()
        print(f"[INIT] no_image=True — h_modal = h_t (texte seul)", flush=True)
    elif params.no_text:
        h_t     = torch.zeros_like(h_t)
        h_modal = h_v.clone()
        print(f"[INIT] no_text=True — h_modal = h_v (image seule)", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            modal_loss  = (torch.tensor(0.0, device=params.device)
                           if (params.no_modal or params.no_image or params.no_text)

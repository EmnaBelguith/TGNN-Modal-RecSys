...
def train(params):
    ...
    _sr_history   = []    # historique des SR nDCG par epoch pour détection du pic
    _topic_frozen = False
    ...
    for iter_idx in range(1, params.epoch):
        ...
        # Freeze topic encoder + topic_linear au pic SR (2 déclins consécutifs)
        # Condition supplémentaire : MI doit être stabilisée avant de freeze
        # (MI < 1.5 ≈ ln(512)/4, et variation < 0.15 sur 10 epochs)
        _mi_stable = (
            len(_mi_history) >= 10 and
            _mi_history[-1] < 1.5 and
            abs(_mi_history[-1] - _mi_history[-10]) < 0.15
        )
        if not _topic_frozen and len(_sr_history) >= 3 and _mi_stable:
            if _sr_history[-1] < _sr_history[-2] and _sr_history[-2] < _sr_history[-3]:
                for p in net.topic_encoder.parameters():
                    p.requires_grad_(False)
                for p in net.topic_decoder.topic_linear.parameters():
                    p.requires_grad_(False)
                _topic_frozen = True
                _sr_peak = max(_sr_history)
                print(f"[FREEZE] ep={iter_idx}  SR_pic={_sr_peak:.4f}  topic encoder + topic_linear gelés", flush=True)
                _freeze_ckpt_path = params.model_save_path.replace('.pt', f'_freeze_ep{iter_idx}.pt')

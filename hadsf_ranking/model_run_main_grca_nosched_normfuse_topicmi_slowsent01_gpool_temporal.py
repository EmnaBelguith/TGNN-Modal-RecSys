...
from rhg_data_temporal import GraphData
...
def config():
    ...
    # ── Config fixe dataset ─────────────────────────────────────────────────
    args.dataset_name = 'Musical_HADSF_temporal'
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}_logdeg_normfuse_topicmi_slowsent01_gpool{_run_tag}.pt'
    )
        ...
        for input_nodes, pos_graph, _neg_graph, blocks in dataloader:
            ...
            for local_i, global_i in enumerate(g_iids):
                if global_i not in item_emb:
                    if self.rating_encoder.item_degree_tensor[global_i].item() == 0:
                        # Cold item : bypass GCN — utilise h_modal si modal dispo, sinon item_embedding (Xavier)
                        _hm = self.rating_encoder.h_modal[global_i]
                        if _hm.norm().item() > 0:
                            item_emb[global_i] = _hm.detach().cpu()
                        else:
                            item_emb[global_i] = self.rating_encoder.item_embedding[global_i].detach().cpu()
                    else:
...
def train(params):
    ...
    _modal_params = list(modal_enc.parameters()) if modal_enc is not None else []
    _other_params = [p for p in list(net.parameters()) + _modal_params
    ...
    if not params.no_modal:
        ...
        net.rating_encoder.h_modal = h_modal
        print(f"[INIT] no_modal — item_embedding garde Xavier init (comme users)", flush=True)

    import pandas as pd
    # interactions_reviews_temporal.csv : train uniquement (avant T), IDs alignés sur _reviews_item2id
    _df_deg_train = pd.read_csv('/home/infres/belguith/PFE/processed/Musical_interactions_reviews_temporal.csv')
    ...
    print("[INIT] item_degree_tensor injected (Musical_interactions_reviews_temporal.csv — train only)", flush=True)

    _diag_deg = _deg_tensor_train
    _diag_cold_idx    = torch.where(_diag_deg == 0)[0]
    _diag_popular_idx = torch.where(_diag_deg  > 0)[0]
    print(f"[DIAG_SETUP] cold_items={len(_diag_cold_idx)} (deg=0), popular_items={len(_diag_popular_idx)} (deg>0)", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            # Cold items (deg=0) absents des mini-batches (pas d'arêtes train) mais ont
            # des features image+text valides → les inclure dans le SSL pour stabiliser h_modal.
            # GRCA délibérément exclu : h_collab ≈ 0 pour deg=0, le gradient serait destructif.
            if not params.no_modal:
                batch_items = torch.cat([batch_items, _diag_cold_idx.to(params.device)])
            ...
            # ── GRCA cold items — approche naturelle ──────────────────────────────────────
            # Cold items (deg=0) absents des mini-batches → GCN output = 0.
            # Approche naturelle : item_embedding[cold] passe par les MÊMES couches FC
            # que h_collab des warm items (agg_act → dropout → ifc).
            # Équivalent à un self-loop GCN pour les nœuds sans voisins.
            # alpha=0 → w=(1-alpha)=1 → plein poids GRCA, f_loss=0 naturellement.
            grca_cold_loss = torch.tensor(0.0, device=params.device)
            if not params.no_modal and params.lambda_grca > 0 and len(_diag_cold_idx) > 0:
                _cold_dev     = _diag_cold_idx.to(params.device)
                _emb_cold     = net.rating_encoder.item_embedding[_cold_dev]   # (N_cold, d)
                # Même transformation que item_outputs warm après GCN : agg_act → dropout → ifc
                _col_cold     = net.rating_encoder.ifc(
                    net.rating_encoder.dropout(
                        net.rating_encoder.agg_act(_emb_cold)))                # (N_cold, d)
                _mod_cold     = h_modal[_cold_dev]                             # (N_cold, d)
                _a_cold       = F.normalize(_col_cold, dim=-1)
                _b_cold       = F.normalize(_mod_cold, dim=-1)
                _logits_cold  = torch.matmul(_a_cold, _b_cold.T) / 0.5
                _lbl_cold     = torch.arange(_logits_cold.shape[0], device=params.device)
                grca_cold_loss = params.lambda_grca * (
                    F.cross_entropy(_logits_cold,   _lbl_cold) +
                    F.cross_entropy(_logits_cold.T, _lbl_cold)
                ) / 2

            loss = r_loss + 0.1 * modal_loss + f_loss + mi_term + grca_loss + grca_cold_loss
...
def test(params):
    import evaluate_model_run_temporal as _em

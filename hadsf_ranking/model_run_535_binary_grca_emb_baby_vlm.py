...
def config():
    ...
    parser.add_argument('--image_feat',        type=str,   default='cnn',
                        choices=['cnn', 'qwen', 'llava', 'qwen_clip'],
                        help='Source features image: cnn=ResNet50 2048-dim, qwen/llava=VLM MiniLM 384-dim, qwen_clip=VLM CLIP 512-dim')
    parser.add_argument('--image_mlp_depth',   type=int,   default=1,
                        help='Profondeur MLP image: 1=Linear seul, 2+=couches 256')
    parser.add_argument('--no_modal_loss',     action='store_true', default=False,
                        help='Désactive InfoNCE(h_v, h_t) — GRCA et f_loss restent actifs')
    ...
    _img_tag  = f'_{args.image_feat}'      if args.image_feat != 'cnn'   else ''
    _mlp_tag  = f'_mlp{args.image_mlp_depth}' if args.image_mlp_depth > 1 else ''
    _nml_tag  = '_noml'                    if args.no_modal_loss          else ''
    args.model_save_path = (
        f'model_save/{args.dataset_name}/{args.model_short_name}'
        f'_layers_{args.num_layers}_seed{args.seed}'
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}{_img_tag}{_mlp_tag}{_nml_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_grca_emb_baby{_run_tag}.pt'
    )
...
class Net(nn.Module):
    ...
    def evaluate_ranking_ndcg(self, dataloader, dataset, K=10,
        ...
        return _m(ndcg_all['global']), _m(ndcg_all['cold']), _m(ndcg_all['medium']), _m(ndcg_all['warm'])
...
def train(params):
    ...
    v_feat, t_feat = load_modal_features('/home/infres/belguith/PFE/bm3_data/baby', params.image_feat)
    modal_enc = ModalEncoder(v_feat, t_feat, embed_dim=params.gcn_out_units, image_mlp_depth=params.image_mlp_depth).to(params.device)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            modal_loss  = (torch.tensor(0.0, device=params.device) if (params.no_modal or params.no_modal_loss)
        ...
        valid_ndcg, valid_cold, valid_medium, valid_warm = net.evaluate_ranking_ndcg(valid_dataloader, dataset, K=10, etype='valid')
        print(f"[VALID_GROUPS] epoch={iter_idx:>3d}  global={valid_ndcg:.4f}  cold={valid_cold:.4f}  medium={valid_medium:.4f}  warm={valid_warm:.4f}", flush=True)
        ...
        if valid_ndcg > best_valid_ndcg:
            ...
            no_better_valid = 0
            best_iter       = iter_idx
            test_ndcg, test_cold, test_medium, test_warm = net.evaluate_ranking_ndcg(test_dataloader, dataset, K=10, etype='test')
        ...
        # checkpoint best cold (indépendant du global)
        ...
        if not _topic_frozen and iter_idx >= params.lambda_mi_warmup:
            if _sr_ndcg > _best_sr_ndcg:
                ...
                _sr_no_improve    = 0
                _best_sr_snapshot = copy.deepcopy(net.topic_decoder.state_dict())
                ...
                if _sr_no_improve >= params.sr_freeze_patience:
                    net.topic_decoder.load_state_dict(
                        {k: v.to(params.device) for k, v in _best_sr_snapshot.items()}
                    )
                    for p in net.topic_decoder.parameters():
                        p.requires_grad_(False)
                    _topic_frozen = True
                    print(f"[SR_FREEZE] Topic encoder gelé @ep{iter_idx} "

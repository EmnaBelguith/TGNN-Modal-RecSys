...
def config():
    ...
    parser.add_argument('--sr_n_neg',          type=int, default=99,
                        help='Nb négatifs random en eval SR (5, 10, 99...)')
...
class SentenceRetrival(nn.Module):
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        """Eval SR : mean(phrases) vs 99 reviews random — identique au training."""
        graph.nodes['user'].data['th'] = utf
        graph.nodes['item'].data['th'] = itf
        def _get(edges):
            dev = next(self.topic_linear.parameters()).device
            th = self.topic_linear(torch.cat([edges.src['th'].to(dev), edges.dst['th'].to(dev)], dim=1))
            return {'th': th}
        graph.apply_edges(_get)
        h          = graph.edata['th']                # (N, dim)
        true_sents = graph.edata['sentence_id']       # (N, n_phrases)
        dev        = h.device
        vocab_size = self.sentence_embedding.weight.shape[0]
        n_phrases  = true_sents.shape[1]
        N          = h.shape[0]

        # Positif — mean des phrases de la vraie review (comme en training)
        pos_emb   = self.get_review_feature(true_sents.to(dev))  # (N, dim)
        pos_score = (h * pos_emb).sum(-1)                         # (N,)

        # n_neg négatifs random — groupes de phrases random (comme en training)
        n_neg = getattr(self, '_sr_n_neg', 99)
        ranks = torch.ones(N, dtype=torch.long, device=dev)  # rang du positif (1 = meilleur)
        for _ in range(n_neg):
            rand_sid  = torch.randint(1, vocab_size, (N, n_phrases), device=dev)
            rand_emb  = self.get_review_feature(rand_sid)    # (N, dim)
            rand_score = (h * rand_emb).sum(-1)              # (N,)
            ranks += (rand_score >= pos_score).long()        # +1 si négatif meilleur

        # nDCG : si rang <= topk, contribution = 1/log2(rang+1)
        ndcg_vals = torch.where(ranks <= topk,
                                1.0 / torch.log2(ranks.float() + 1),
                                torch.zeros_like(ranks.float()))
        hr_vals   = (ranks <= topk).float()
        pre_vals  = hr_vals / topk
        rec_vals  = hr_vals  # 1 positif → recall = hr

        return {
            'Pre':  pre_vals.cpu().tolist(),
            'Rec':  rec_vals.cpu().tolist(),
            'F1':   (2 * pre_vals * rec_vals / (pre_vals + rec_vals + 1e-9)).cpu().tolist(),
            'nDCG': ndcg_vals.cpu().tolist(),
        }
...
class Net(nn.Module):
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        """Eval SR : mean(phrases_item_i) vs mean(phrases_item_j) pour j != i.
        Passe 1 : collecter tous les h et embeddings de reviews.
        Passe 2 : ranker chaque positif contre 99 reviews d'autres items réels.
        """
        device  = self.review_embedding.weight.device
        n_neg   = getattr(self, '_sr_n_neg', 99)

        # — Passe 1 : collecter h et review embeddings pour toutes les edges —
        all_h        = []
        all_pos_emb  = []
        all_ratings  = []
        for rating_input_nodes, pos_graph, _neg_graph, rating_encoder_blocks in dataloader:
            ...
            # Calcul de h = topic_linear(utf, itf)
            decoder_graph.nodes['user'].data['th'] = utf
            decoder_graph.nodes['item'].data['th'] = itf
            def _get_h(edges):
                dev = next(self.topic_decoder.topic_linear.parameters()).device
                return {'_h': self.topic_decoder.topic_linear(
                    torch.cat([edges.src['th'].to(dev), edges.dst['th'].to(dev)], dim=1))}
            decoder_graph.apply_edges(_get_h)
            h        = decoder_graph.edata['_h']                          # (N, dim)
            true_sid = decoder_graph.edata['sentence_id']                  # (N, n_phrases)
            pos_emb  = self.topic_decoder.get_review_feature(true_sid.to(device))  # (N, dim)

            all_h.append(h.cpu())
            all_pos_emb.append(pos_emb.cpu())
            all_ratings.extend(decoder_graph.edata['rating'].cpu().tolist())

        all_h       = torch.cat(all_h, dim=0)        # (N_total, dim)
        all_pos_emb = torch.cat(all_pos_emb, dim=0)  # (N_total, dim)
        N_total     = all_h.shape[0]

        # — Passe 2 : ranker chaque positif contre n_neg reviews d'autres items —
        all_pos_scores = (all_h * all_pos_emb).sum(-1)  # (N_total,)
        ranks = torch.ones(N_total, dtype=torch.long)
        for i in range(N_total):
            # Sampler n_neg autres edges (autres items)
            neg_idx = torch.randint(0, N_total, (n_neg,))
            neg_idx[neg_idx == i] = (i + 1) % N_total  # éviter self
            neg_scores = (all_h[i] * all_pos_emb[neg_idx]).sum(-1)  # (n_neg,)
            ranks[i]   = (neg_scores >= all_pos_scores[i]).sum() + 1
        ...
        scores_list = [{'Pre': [pre_vals[i].item()], 'Rec': [rec_vals[i].item()],
                        'F1': [0.0], 'nDCG': [ndcg_vals[i].item()]} for i in range(N_total)]
        group_scores = defaultdict(lambda: defaultdict(list))
        for i, rating in enumerate(all_ratings):
            g = int(rating)
            group_scores[g]['Pre'].append(pre_vals[i].item())
            group_scores[g]['Rec'].append(rec_vals[i].item())
            group_scores[g]['nDCG'].append(ndcg_vals[i].item())
        ...
        return {'Pre': pre_vals.mean().item(), 'Rec': rec_vals.mean().item(),
                'F1': 0.0, 'nDCG': ndcg_vals.mean().item()}

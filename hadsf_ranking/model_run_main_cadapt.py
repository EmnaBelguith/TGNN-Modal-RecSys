...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        _gate_input = torch.cat([_pre_ci_norm_scaled, _log_deg_norm], dim=-1)
        alpha = torch.sigmoid(F.softplus(self.gate_temp) * self.gate_mlp(_gate_input))
        if self.training:
        ...
        _collab_adapted = self.modal_adapter(torch.cat([item_outputs, _item_modal_out], dim=-1))
        item_outputs = alpha * _collab_adapted + (1 - alpha) * _item_modal_out
...
class Net(nn.Module):
    ...
    def _get_item_emb_global(self, global_iids):
        ...
        alpha     = torch.sigmoid(F.softplus(self.rating_encoder.gate_temp) * self.rating_encoder.gate_mlp(gate_in))
        _ca       = self.rating_encoder.modal_adapter(torch.cat([collab, modal], dim=-1))
        emb       = alpha * _ca + (1 - alpha) * modal

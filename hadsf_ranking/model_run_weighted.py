...
class Net(nn.Module):
    ...
    def evaluate_rating(self, dataloader, etype='valid'):
        ...
        for input_nodes, edge_subgraph, blocks in dataloader:
            ...
            # Collecte (item_id, pred, true) pour cold-start
            if etype == 'test':
                if not hasattr(self, '_cs_buffer'): self._cs_buffer = []
                try:
                    _, dst_idx = edge_subgraph.edges()
...
# =====================
    ...
    with torch.no_grad():
        net.rating_encoder.item_embedding.data.copy_(h_modal[:net.rating_encoder.item_embedding.shape[0]])
    print("[INIT] item_embedding initialisé depuis h_modal", flush=True)
    import pandas as pd
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, edge_subgraph, rating_blocks in pbar:
            ...
            # Cold-start : items fort collab → plus de collab (alpha élevé)
            cold_reg = torch.tensor(0.0, device=params.device)
            if hasattr(net.rating_encoder, '_last_alpha'):
                _alpha_train   = net.rating_encoder._last_alpha
                _degrees_train = net.rating_encoder._last_degrees
                _target_alpha  = torch.sigmoid(2.0 * torch.log1p(_degrees_train) - 5.0)
                cold_reg = F.mse_loss(_alpha_train, _target_alpha)
            loss = r_loss + 0.1 * modal_loss + 0.05 * cold_reg
            optimizer.zero_grad()
            loss.backward()
            ...
            pbar.set_description(f"train_loss={r_loss:.4f}, MI={mi_score:.2f}, ranking={ranking_loss.item():.4f}, modal={modal_loss.item():.4f}")
...
def test(params):
    ...
    # Ranking evaluation — Approche 1
    if hasattr(net, '_ranking_buffer') and net._ranking_buffer:
        from collections import defaultdict
        user_preds = defaultdict(list)
        # Groupe par item (proxy user via cold-start buffer)
        item_preds = {}
        for iid, pred, true in net._ranking_buffer:
            item_preds[iid] = (pred, true)

        # Calcul nDCG@K sur les items du test par user
        import json
        test_by_user = defaultdict(list)
        with open('/home/infres/belguith/PFE/processed/Musical_reviews_with_aspects_test.json') as f:
            for line in f:
                d = json.loads(line)
                test_by_user[d['user_id']].append((d['item_id'], d['rating']))

        ndcg_10, recall_10, ndcg_20, recall_20 = [], [], [], []
        for uid, items in test_by_user.items():
            # Items du test avec leurs scores prédits
            scored = [(iid, item_preds[iid][0], rating)
                      for iid, rating in items if iid in item_preds]
            if not scored: continue
            positives = set(iid for iid, _, r in scored if r >= 3)
            if not positives: continue
            ranked = sorted(scored, key=lambda x: x[1], reverse=True)
            ranked_ids = [x[0] for x in ranked]
            for K, ndcg_list, recall_list in [(10, ndcg_10, recall_10), (20, ndcg_20, recall_20)]:
                dcg = sum(1.0/__import__('math').log2(i+2)
                          for i,iid in enumerate(ranked_ids[:K]) if iid in positives)
                ideal = sum(1.0/__import__('math').log2(i+2)
                            for i in range(min(len(positives),K)))
                ndcg_list.append(dcg/ideal if ideal>0 else 0)
                hits = sum(1 for iid in ranked_ids[:K] if iid in positives)
                recall_list.append(hits/len(positives))

        import numpy as np
        print(f"\n[RANKING] threshold=3 (rating>=3 → positif)")
        print(f"  nDCG@10  = {np.mean(ndcg_10):.4f}")
        print(f"  Recall@10= {np.mean(recall_10):.4f}")
        print(f"  nDCG@20  = {np.mean(ndcg_20):.4f}")
        print(f"  Recall@20= {np.mean(recall_20):.4f}")
        print(f"  Users évalués = {len(ndcg_10)}")

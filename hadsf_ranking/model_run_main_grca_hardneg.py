...
class SentenceRetrival(nn.Module):
    ...
    def calc_sentence_ranking(self, edges):
        """Phrase individuelle vs phrase individuelle du même item — exactement comme SR eval."""
        ...
        pos_sid  = edges.data['sentence_id']                  # (N, n_phrases)
        cand_sid = edges.dst['candidate_sentence_id']         # (N, n_cands)
        N        = th.shape[0]

        # Positif — UNE phrase choisie aléatoirement dans la vraie review
        n_phrases   = pos_sid.shape[1]
        pos_idx     = torch.randint(0, n_phrases, (N,), device=pos_sid.device)
        pos_phrase  = pos_sid[torch.arange(N, device=pos_sid.device), pos_idx]  # (N,)
        pos_emb     = self.sentence_embedding(pos_phrase)     # (N, dim)
        pos_score   = (th * pos_emb).sum(-1)                  # (N,)

        # Négatif — UNE phrase du même item (candidate_sentence_id)
        n_cands    = cand_sid.shape[1]
        neg_idx    = torch.randint(0, n_cands, (N,), device=pos_sid.device)
        neg_phrase = cand_sid[torch.arange(N, device=pos_sid.device), neg_idx]  # (N,)

        # Fallback random si padding (sid=0)
        has_cands  = (cand_sid > 0).any(dim=1)
        neg_rand   = torch.randint(1, self.sentence_embedding.weight.shape[0], (N,), device=pos_sid.device)
        neg_phrase = torch.where(has_cands, neg_phrase, neg_rand)

        neg_emb    = self.sentence_embedding(neg_phrase)      # (N, dim)
        neg_score  = (th * neg_emb).sum(-1)                   # (N,)
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        _formula = getattr(self, '_sr_formula', 'topic_only')
        if _formula in ('vrai_fix', 'orig'):
            graph.nodes['user'].data['th'] = utf + urf
            graph.nodes['item'].data['th'] = itf + irf
        else:
        ...
        def _get(edges):
            ...
            if _formula in ('fix1', 'vrai_fix'):
                rh = self.rating_linear(torch.cat([edges.src['rf'].to(dev), edges.dst['rf'].to(dev)], dim=1))
                th = th + rh
            # orig : topic_linear(utf+urf, itf+irf) sans rating_linear — ancienne formule buggy
            return {'th': th, 'cand_sid': edges.dst['candidate_sentence_id']}

...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_run_tag}_gatefull.pt'
    )
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        # pre-ci full embedding: MLP voit le vecteur 128-dim complet avant normalisation ci
        # magnitude ∝ sqrt(degree) → le MLP peut déduire cold/warm depuis les valeurs elles-mêmes
        _last_conv  = self.conv_layers[-1]
        _pre_ci_sum = sum(_last_conv[r]._last_pre_ci for r in self.rating_values)
        _pre_ci_norm_scaled = _pre_ci_sum.norm(dim=-1, keepdim=True) / \
                              (_pre_ci_sum.detach().norm(dim=-1).max().clamp(min=1e-8))  # pour cache
        _global_scale = _pre_ci_sum.detach().abs().max().clamp(min=1e-8)
        _gate_input   = _pre_ci_sum / _global_scale   # 128-dim, magnitudes relatives préservées
        alpha = torch.sigmoid(self.gate_mlp(_gate_input))
        if self.training:
...
class Net(nn.Module):
    ...
    def _get_item_emb_global(self, global_iids):
        ...
        gate_in   = torch.cat([pre_ci_n, log_deg_n], dim=-1)
...
def train(params):
    ...
    with torch.no_grad():
        ...
        _rand_emb = F.normalize(torch.randn(_n_items, params.gcn_out_units, device=params.device), dim=-1) * _modal_avg_norm
        net.rating_encoder.item_embedding.data.copy_(_rand_emb)
    print(f"[INIT] item_embedding random at norm={_modal_avg_norm:.3f}", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            pbar.set_description(
                ...
                f"mi_term={mi_term.item():.4f}, modal={modal_loss.item():.4f}, f_loss={f_loss.item():.6f}"
            )
        ...
        with torch.no_grad():
            ...
            if _acc:
                ...
                print(f"[GATE_CORR] alpha vs degree={_corr:.4f}", flush=True)

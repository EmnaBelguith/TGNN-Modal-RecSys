...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_topiconly_eval{_run_tag}.pt'
    )
...
class SentenceRetrival(nn.Module):
        ...
        with graph.local_scope():
            if neg_strategy == 'inbatch':
                def _store(edges):
                    ...
                    fused_u = edges.src['tf'] + edges.src['rf']
                    fused_i = edges.dst['tf'] + edges.dst['rf']
                    th_val = self.topic_linear(torch.cat([fused_u, fused_i], dim=1)) + rh
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        # eval : topic seul — pas d'urf/irf pour éviter la dérive BPR
        graph.nodes['user'].data['tf'] = utf
        graph.nodes['item'].data['tf'] = itf

...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_warmadapt{_run_tag}.pt'
    )
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def __init__(self, rating_values, review_embedding, user_size, item_size, msg_units, num_layers, aggregate='sum', dropout_rate=0.0):
        ...
        # MLP de fusion warm : apprend à combiner collab + modal pour les warm items
        self.modal_adapt_mlp = nn.Sequential(
            nn.Linear(2 * msg_units, msg_units),
            nn.ReLU(),
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        # residuel cold : (1-α)·h_modal   — domine pour cold (α≈0.32 → 1-α≈0.68)
        # bonus warm    : α·MLP(cat[c,m]) — domine pour warm (α≈0.81)
        _adapt = self.modal_adapt_mlp(torch.cat([item_outputs, _item_modal_out], dim=-1))
        item_outputs = item_outputs + (1 - alpha) * _item_modal_out + alpha * _adapt
...
class Net(nn.Module):
    ...
    def _get_item_emb_global(self, global_iids):
        ...
        _adapt = self.rating_encoder.modal_adapt_mlp(torch.cat([collab, modal], dim=-1))
        emb    = collab + (1 - alpha) * modal + alpha * _adapt

...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_normfuse_freeze{_run_tag}.pt'
    )
...
class SentenceRetrival(nn.Module):
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        # SR eval : topic seul (utf/itf) — stable après freeze car topic_encoder+sentence_embedding gelés
        # urf/irf exclus : SR mesure le topic pur, indépendamment du graphe collaboratif
        graph.nodes['user'].data['tf'] = utf
        graph.nodes['item'].data['tf'] = itf
...
def train(params):
    ...
    for iter_idx in range(1, params.epoch):
        ...
        if params.lambda_mi > 0 and _epoch_mi_count > 0:
            ...
            _mi_history.append(_epoch_avg_mi)
        ...
        _sr_ndcg = float(np.mean(_sr_result['nDCG']))
        _sr_history.append(_sr_ndcg)

        # Snapshot des poids topic si meilleur SR jusqu'ici
        if not _topic_frozen and _sr_ndcg > _best_sr_ndcg:
            _best_sr_ndcg = _sr_ndcg
            _best_sr_ep   = iter_idx
            _best_topic_state = {
                'topic_encoder': copy.deepcopy(net.topic_encoder.state_dict()),
                'topic_linear':  copy.deepcopy(net.topic_decoder.topic_linear.state_dict()),
                'sentence_emb':  net.sentence_embedding.weight.data.clone(),
            }
            print(f"[BEST_SR] ep={iter_idx}  SR={_sr_ndcg:.4f}  snapshot topic sauvegardé", flush=True)

        # Freeze au 1er déclin SR — restaure le snapshot du vrai pic
        if not _topic_frozen and len(_sr_history) >= 2:
            if _sr_history[-1] < _sr_history[-2]:
                # Restaurer les poids du meilleur SR
                net.topic_encoder.load_state_dict(_best_topic_state['topic_encoder'])
                net.topic_decoder.topic_linear.load_state_dict(_best_topic_state['topic_linear'])
                net.sentence_embedding.weight.data.copy_(_best_topic_state['sentence_emb'])
                # Geler
                for p in net.topic_encoder.parameters():
                    p.requires_grad_(False)
                for p in net.topic_decoder.topic_linear.parameters():
                    p.requires_grad_(False)
                net.sentence_embedding.weight.requires_grad_(False)
                _topic_frozen = True
                print(f"[FREEZE] ep={iter_idx}  SR_pic={_best_sr_ndcg:.4f} @ep{_best_sr_ep}  restauré+gelé (topic_encoder+topic_linear+sentence_emb)", flush=True)
                _freeze_ckpt_path = params.model_save_path.replace('.pt', f'_freeze_ep{_best_sr_ep}.pt')
                _freeze_ckpt = net.state_dict()
                _freeze_ckpt['modal_enc'] = modal_enc.state_dict()
                torch.save(_freeze_ckpt, _freeze_ckpt_path)
                print(f"[FREEZE] checkpoint sauvegardé → {_freeze_ckpt_path}", flush=True)
        net.train()

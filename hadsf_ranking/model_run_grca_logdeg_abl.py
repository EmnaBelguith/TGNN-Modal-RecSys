...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nimg_tag}{_ntxt_tag}{_grca_tag}_logdeg{_run_tag}.pt'
    )
...
def train(params):
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            # InfoNCE requiert les deux modalités
            _no_infonce = params.no_modal or params.no_image or params.no_text
            # f_loss et GRCA fonctionnent avec une seule modalité
            _no_floss   = params.no_modal
            modal_loss  = (torch.tensor(0.0, device=params.device) if _no_infonce
            ...
            if (not _no_floss and params.lambda_grca > 0
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                if modal_enc.use_image and modal_enc.use_text:
                    ...
                    _hm = net.rating_encoder.h_modal
                    print(f"[EPOCH_MODAL] epoch={iter_idx} "
                          f"single_mod={'img' if modal_enc.use_image else 'txt'} "
                          f"h_modal_norm={_hm.norm(dim=-1).mean():.3f}", flush=True)

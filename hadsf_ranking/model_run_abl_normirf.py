...
class Net(nn.Module):
    ...
    def calc_loss(self,
        ...
        # F.normalize : scores bornés → sigmoid jamais saturée → toutes paires contribuent
        urf_n = F.normalize(urf, dim=-1)
        irf_n = F.normalize(irf, dim=-1)
        u_emb     = urf_n[src_pos]
        i_pos_emb = irf_n[dst_pos]

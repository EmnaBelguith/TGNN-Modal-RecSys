...
def config():
    ...
    parser.add_argument('--image_feat',        type=str,   default='qwen',
                        choices=['cnn', 'qwen', 'llava', 'qwen_clip'],
                        help='Source des features image: cnn=ResNet50 2048-dim, qwen/llava=VLM MiniLM 384-dim, qwen_clip=VLM CLIP 512-dim')
    parser.add_argument('--image_mlp_depth',   type=int,   default=1,
                        help='Profondeur du MLP image: 1=Linear seul, 2=Linear(D→256)→ReLU→Linear(256→128), 3=+couche 256')
    parser.add_argument('--no_modal_loss',     action='store_true', default=False,
                        help='Désactive la loss InfoNCE(h_v, h_t) — GRCA et f_loss restent actifs')
    ...
    _nml_tag   = '_nomolloss'            if args.no_modal_loss            else ''
    args.model_save_path = (
        f'model_save/{args.dataset_name}/{args.model_short_name}'
        f'_layers_{args.num_layers}_seed{args.seed}'
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}{_rinit_tag}{_bprw_tag}{_img_tag}{_mlp_tag}{_nml_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_grca_emb{_run_tag}.pt'
    )
...
def train(params):
    ...
    v_feat, t_feat = load_modal_features('/home/infres/belguith/PFE/bm3_data/musical', params.image_feat)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            modal_loss  = (torch.tensor(0.0, device=params.device)
                           if (params.no_modal or params.no_modal_loss)

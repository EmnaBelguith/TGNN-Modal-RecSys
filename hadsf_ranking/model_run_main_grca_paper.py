...
class SentenceRetrival(nn.Module):
    ...
    def calc_sentence_ranking(self, edges):
        """Formule papier exacte : h = h_r + h_t = (urf+irf) + (utf+itf).
        NB: tf est passé comme utf+urf dans calc_loss → tf_src+tf_dst = urf+irf+utf+itf ✓"""
        h = edges.src['tf'] + edges.dst['tf']  # (utf+urf) + (itf+irf) = h_r + h_t  (N, dim)
        pos_sid    = edges.data['sentence_id']
        pos_review = self.get_review_feature(pos_sid)          # mean(phrases) → (N, dim)
        pos_score  = (h * pos_review).sum(1)
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        """Formule papier exacte : h = urf + irf + utf + itf (somme directe)."""
        graph.nodes['user'].data['rf'] = urf
        graph.nodes['item'].data['rf'] = irf
        graph.nodes['user'].data['tf'] = utf
        graph.nodes['item'].data['tf'] = itf
        def _get(edges):
            h = (edges.src['rf'] + edges.dst['rf'] +
                 edges.src['tf'] + edges.dst['tf'])   # h = h_r + h_t

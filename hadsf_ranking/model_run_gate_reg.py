...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def __init__(self, rating_values, review_embedding, user_size, item_size, msg_units, num_layers, aggregate='sum', dropout_rate=0.0):
        ...
        self.modal_gate = nn.Sequential(
            nn.Linear(msg_units * 2, msg_units),
            nn.ReLU(),
            nn.Linear(msg_units, msg_units)
        )
        ...
        for l in range(len(self.conv_layers)):
            ...
            for rating in self.rating_values:
                ...
                if l == 0: 
                    ...
                    alpha = torch.sigmoid(self.modal_gate(gate_input))
                    self.last_alpha = (input_nodes['item'], alpha)  # expose pour gate_reg_loss
...
# =====================
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, edge_subgraph, rating_blocks in pbar:
            ...
            # gate reg loss : force alpha ~ degre normalise
            if hasattr(net.rating_encoder, "last_alpha"):
                item_ids, alpha_batch = net.rating_encoder.last_alpha
                degree_target = torch.sigmoid(item_degrees[item_ids.cpu()].to(params.device))  # [B]
                alpha_mean = alpha_batch.mean(dim=-1)  # [B]
                gate_reg = F.mse_loss(alpha_mean, degree_target)
            else:
                gate_reg = torch.tensor(0.0, device=params.device)

            loss = r_loss + 0.1 * modal_loss + 0.1 * gate_reg
            optimizer.zero_grad()
            loss.backward()
        ...
        with torch.no_grad():
            ...
            _sim = torch.nn.functional.cosine_similarity(net.rating_encoder.h_modal[:_n], net.rating_encoder.item_embedding).mean()
            print(f"[DIAG] cosine sim h_modal/collab = {_sim:.4f}", flush=True)
            # DIAG cosine similarity
            _sim = F.cosine_similarity(net.rating_encoder.h_modal[:_n], net.rating_encoder.item_embedding).mean()
            print(f"[DIAG] cosine sim h_modal/collab = {_sim:.4f}", flush=True)
            # DIAG cosine similarity
            _sim = F.cosine_similarity(net.rating_encoder.h_modal[:_n], net.rating_encoder.item_embedding).mean()
            print(f"[DIAG] cosine sim h_modal/collab = {_sim:.4f}", flush=True)

...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_logdegw{_run_tag}.pt'
    )
...
class Net(nn.Module):
    ...
    def calc_loss(self,
        ...
        # Log-degree BPR : pondérer par log(max_deg+1)/log(deg+1) — boost doux pour cold
        _pos_global = pos_graph.nodes['item'].data['_ID'][dst_pos]
        _pos_deg    = self.rating_encoder.item_degree_tensor[_pos_global].float().clamp(min=1)
        _max_deg    = self.rating_encoder.item_degree_tensor.float().max().clamp(min=1)
        _w          = torch.log(_max_deg + 1) / torch.log(_pos_deg + 1)
        _w          = _w / _w.mean()   # normaliser → amplitude loss stable
        ...
        bpr_loss = -(_w * torch.stack(bpr_terms).mean(0)).mean()

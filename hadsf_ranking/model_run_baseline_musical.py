...
def config():
    ...
    args.model_short_name = 'RHGC4_baseline'
...
def test(params):
    ...
    net.item_degree_groups = (
        set(_deg_train[(_deg_train >= 5)  & (_deg_train <= 10)].index),
        set(_deg_train[(_deg_train >= 11) & (_deg_train <= 20)].index),
        set(_deg_train[_deg_train > 20].index)
    )
    net._cs_buffer = []
    test_rmse,test_mae,test_mse = net.evaluate_rating(test_dataloader, etype='test')

...
# =====================
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            # InfoNCE requiert les deux modalités — désactivée si l'une manque
            _no_infonce = params.no_modal or params.no_image or params.no_text
            # f_loss fonctionne avec une seule modalité — désactivée seulement si no_modal total
            _no_floss   = params.no_modal
            modal_loss  = torch.tensor(0.0, device=params.device) if _no_infonce else \
            ...
            if not _no_floss and _mask_pos.sum() > 0 and _mask_neg.sum() > 0:

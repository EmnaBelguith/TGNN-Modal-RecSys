...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_grca_emb_baby{_run_tag}.pt'
    )
...
def train(params):
    ...
    _train_u, _train_i = dataset.graph['interacts'].edges()
    _deg_tensor_train = torch.zeros(_n_init, dtype=torch.float32)
    _deg_tensor_train.scatter_add_(0, _train_i.long(), torch.ones(_train_i.shape[0]))
    net.rating_encoder.item_degree_tensor = _deg_tensor_train.to(params.device)
    print("[INIT] item_degree_tensor injected (graph['interacts'] train edges, IDs 0..5491)", flush=True)

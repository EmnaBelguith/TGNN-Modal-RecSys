...
def config():
    ...
    parser.add_argument('--phase2_ckpt',   type=str, default='',
                        help='Path to phase 1 checkpoint to load before phase 2 training')
    parser.add_argument('--cold_threshold', type=int, default=10,
                        help='Degree threshold: items with degree <= threshold are cold (gradient allowed)')
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_phase2{_run_tag}.pt'
    )
...
def train(params):
    ...
    import pandas as pd
    # ── Phase 2 : charger checkpoint Phase 1 + geler warm items ───────────────
    if params.phase2_ckpt:
        _map = params.device if torch.cuda.is_available() else 'cpu'
        _ckpt = torch.load(params.phase2_ckpt, weights_only=False, map_location=_map)
        ...
        print(f"[PHASE2] Checkpoint Phase 1 chargé : {params.phase2_ckpt}", flush=True)
    ...
    # ── Phase 2 : gradient hook — geler les warm items (degree > cold_threshold) ──
    if params.phase2_ckpt:
        _cold_mask_1d = (_diag_deg <= params.cold_threshold).float().to(params.device)
        def _freeze_warm_hook(grad):
            mask = _cold_mask_1d
            for _ in range(grad.dim() - 1):
                mask = mask.unsqueeze(-1)
            return grad * mask
        net.rating_encoder.item_embedding.register_hook(_freeze_warm_hook)
        n_frozen = int((_diag_deg > params.cold_threshold).sum().item())
        print(f"[PHASE2] {n_frozen} warm items gelés (degree > {params.cold_threshold}), "
              f"{len(_diag_cold_idx)} cold items libres", flush=True)

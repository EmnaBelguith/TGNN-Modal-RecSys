...
def config():
    ...
    parser.add_argument('--lambda_warmadapt',  type=float, default=0.1,
                        help='Poids du bonus warm : α·ranking_loss(MLP(cat[collab,h_modal])) pour items degré>10')
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_warmbonusloss{_run_tag}.pt'
    )
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        # alpha = degré normalisé directement — cold→0, warm→1, pas de MLP
...
class Net(nn.Module):
    ...
    def _get_item_emb_global(self, global_iids):
        ...
        emb       = collab  # forward = GCN pur, pas de fusion explicite
...
def train(params):
    ...
    print(f"[INIT] item_embedding ← h_modal (semantic init, cosine_sim=1.0)", flush=True)

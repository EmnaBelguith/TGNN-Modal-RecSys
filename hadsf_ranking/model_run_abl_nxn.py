...
class Net(nn.Module):
    ...
    def calc_loss(self,
        ...
        # N×N BPR : dot(urf_i, irf_j) pour toutes les paires du batch
        pos_scores = (u_emb * i_pos_emb).sum(1)              # (N,) diagonal
        score_mat  = u_emb @ i_pos_emb.T                     # (N, N) toutes paires
        diag       = torch.eye(N, dtype=torch.bool, device=u_emb.device)
        score_mat  = score_mat.masked_fill(diag, float('-inf'))
        pos_exp    = pos_scores.unsqueeze(1).expand(N, N)
        bpr_mat    = -F.logsigmoid(pos_exp - score_mat)
        bpr_mat    = bpr_mat.masked_fill(diag, 0.0)
        ...
        bpr_loss = (weight * bpr_mat.sum(1) / (N - 1)).mean()
        ...
        if self._bpr_step <= 10 or self._bpr_step % 100 == 0:
            with torch.no_grad():
                ...
                print(
                    ...
                    f" | pos_score mean={pos_scores.detach().mean():.4f}"
    ...
    def evaluate_ranking_ndcg(self, dataloader, dataset, K=10,
        ...
        # 3. Scorer dot product — cohérent avec le train N×N
        def score_pairs(u_emb_t, i_embs_t):
            return (u_emb_t.unsqueeze(0) * i_embs_t).sum(1)

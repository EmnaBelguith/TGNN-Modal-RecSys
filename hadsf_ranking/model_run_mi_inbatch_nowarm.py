...
class SentenceRetrival(nn.Module):
    ...
    def forward(self, graph, urf, irf, utf, itf, neg_strategy='random', n_neg=1,
                utf_pure=None, itf_pure=None):
        graph.nodes['user'].data['rf'] = urf
        graph.nodes['item'].data['rf'] = irf
        graph.nodes['user'].data['tf'] = utf
        graph.nodes['item'].data['tf'] = itf
        # Découplage MI : topic pur sans signal GCN
        if utf_pure is not None:
            graph.nodes['user'].data['tf_p'] = utf_pure
            graph.nodes['item'].data['tf_p'] = itf_pure
        with graph.local_scope():
            if neg_strategy == 'inbatch':
                def _store(edges):
                    # eval path = topic_linear(cat[tf, tf]) où tf = utf+urf stocké sur le graphe
                    # (identique à get_ranking_scores qui reçoit utf+urf)
                    th_eval = self.topic_linear(torch.cat([edges.src['tf'], edges.dst['tf']], dim=1))
                    if 'tf_p' in edges.src:
                        # th_mi = topic seul, 0% GCN — découplé
                        th_val = self.topic_linear(torch.cat([edges.src['tf_p'], edges.dst['tf_p']], dim=1))
                        return {'_th': th_val, '_rh': torch.zeros_like(th_val), '_th_topic': th_val, '_th_eval': th_eval, '_pids': edges.data['sentence_id']}
                    rh     = self.rating_linear(torch.cat([edges.src['rf'], edges.dst['rf']], dim=1))
                    th_val = self.topic_linear(torch.cat([edges.src['tf'], edges.dst['tf']], dim=1))
                    return {'_th': th_val + rh, '_rh': rh, '_th_topic': th_val, '_th_eval': th_eval, '_pids': edges.data['sentence_id']}
                graph.apply_edges(_store)
                th       = graph.edata['_th']
                rh_all   = graph.edata['_rh']
                th_topic = graph.edata['_th_topic']
                th_eval  = graph.edata.get('_th_eval', th)
                pos_sid = graph.edata['_pids']
                pos_review = self.get_review_feature(pos_sid)
                N = th.shape[0]
                # BPR + F.normalize — scale-invariant, k shuffles in-batch
                th_n      = F.normalize(th, dim=-1)
                pos_rev_n = F.normalize(pos_review, dim=-1)
                pos_score = (th_n * pos_rev_n).sum(1)
                losses = []
                for _ in range(n_neg):
                    perm = torch.randperm(N, device=th.device)
                    clash = perm == torch.arange(N, device=th.device)
                    if clash.any():
                        perm[clash] = (perm[clash] + 1) % N
                    neg_rev_n = pos_rev_n[perm]
                    neg_score = (th_n * neg_rev_n).sum(1)
                    losses.append(-F.logsigmoid(pos_score - neg_score))
                mi = torch.stack(losses).mean(0).mean()
                ...
                if self._mi_step % 200 == 1:
                    with torch.no_grad():
                        ...
                        rev_n   = pos_review.norm(dim=-1).mean().item()
                        # cosine th_train vs pos_review (qualité MI)
                        cos_pos = pos_score.mean().item()
                        # cosine th_train vs th_eval (mismatch train/eval)
                        cos_mismatch = F.cosine_similarity(
                            F.normalize(th, dim=-1),
                            F.normalize(th_eval, dim=-1), dim=-1
                        ).mean().item()
                        # cosine th_eval vs pos_review (ce que SR nDCG mesure vraiment)
                        th_eval_n = F.normalize(th_eval, dim=-1)
                        cos_eval_rev = F.cosine_similarity(th_eval_n, pos_rev_n, dim=-1).mean().item()
                        print(
                            f"[MI_DIAG] step={self._mi_step:>5d} | "
                            f"rh_norm={rh_n:.3f}  topic_norm={topic_n:.3f}  th_norm={th_n_:.3f}  rev_norm={rev_n:.3f} | "
                            f"cos(th_train,rev)={cos_pos:.3f}  cos(th_eval,rev)={cos_eval_rev:.3f}  cos(train,eval)={cos_mismatch:.3f}",
                            flush=True
                        )
                return mi, mi
            else:
...
class Net(nn.Module):
    ...
    def calc_loss(self,
        ...
        ed_mi, ranking_loss = self.topic_decoder(
            ...
            utf_pure=utf, itf_pure=itf   # MI découplée : topic pur, 0% GCN
        )

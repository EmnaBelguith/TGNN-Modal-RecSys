...
def config():
    ...
    parser.add_argument('--use_bestcold',  action='store_true', default=False,
                        help='Load _bestcold.pt checkpoint instead of best global')
    parser.add_argument('--cold_threshold',  type=int, default=10,
                        help='Degree threshold: items with degree <= threshold are cold')
    parser.add_argument('--freeze_patience', type=int, default=10,
                        help='Nb epochs sans amélioration non-cold avant freeze')
    ...
    # ── Config fixe dataset ─────────────────────────────────────────────────
    args.dataset_name = 'Baby_HADSF'
    args.dataset_path = '/home/infres/belguith/PFE/processed/Baby_reviews_with_aspects.jsonl'
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_baby_adaptfreeze{_run_tag}.pt'
    )
...
class SentenceRetrival(nn.Module):
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        ...
        if hasattr(self, '_has_topic'):
            topic_mask = self._has_topic[pos_s.cpu().clamp(max=len(self._has_topic)-1)]          # (E,) bool
...
class Net(nn.Module):
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_encoder_blocks in dataloader:
            ...
            if has_topic is not None:
                tmask = has_topic[pos_s.cpu().clamp(max=len(has_topic)-1)]
    ...
    def evaluate_ranking_ndcg(self, dataloader, dataset, K=10,
        ...
        # degré train pour groupes cold/medium/warm
        item_deg = defaultdict(int)
        for items_u in train_seen.values():
            for i in items_u:
                item_deg[i] += 1
        ...
        ndcg_all = {'global': [], 'cold': [], 'medium': [], 'warm': []}
        ...
        noncold_list = ndcg_all['medium'] + ndcg_all['warm']
        return _m(ndcg_all['global']), _m(ndcg_all['cold']), _m(noncold_list), _m(ndcg_all['medium']), _m(ndcg_all['warm'])
...
def train(params):
    ...
    v_feat, t_feat = load_modal_features('/home/infres/belguith/PFE/bm3_data/baby')
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params': _sent_params,   'lr': params.train_lr * 0.1},
    ])
    print(f"[LR] No scheduler — LR fixe={params.train_lr}  sent_emb_lr={params.train_lr*0.1} (×0.1)", flush=True)
    ...
    no_better_valid      = 0
    best_iter            = -1
    best_cold_valid_ndcg = 0.0
    best_cold_iter       = -1
    cold_save_path       = params.model_save_path.replace('.pt', '_bestcold.pt')
    learning_rate   = params.train_lr
    repr_norm_history = []

    # ── Adaptive freeze state ─────────────────────────────────────────────────
    _frozen              = False
    _best_noncold_valid  = 0.0
    _no_better_noncold   = 0
    _best_cold_postfreeze = 0.0
    _no_better_cold_pf   = 0
    ...
    import pandas as pd
    # Baby_interactions_reviews.csv : IDs alignés sur Musical_reviews_item2id.json (0..4885)
    _df_deg_train = pd.read_csv('/home/infres/belguith/PFE/processed/Baby_interactions_reviews.csv')
    ...
    print("[INIT] item_degree_tensor injected (Baby_interactions_reviews.csv, IDs 0..4885)", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            _eff_lambda_mi = 0.0 if getattr(params, 'no_review', False) else (
        ...
        with torch.no_grad():
            ...
            # cosine sim par groupe cold/warm
            if not params.no_modal and net.rating_encoder.item_degree_tensor is not None:
                _deg   = net.rating_encoder.item_degree_tensor.cpu()
                _h_cpu = _h.cpu()
                _e_cpu = _e.cpu()
                _cold_mask = (_deg <= 10) & (_deg >= 1)
                _warm_mask = (_deg > 20)
                for _mask, _lbl in [(_cold_mask, 'cold'), (_warm_mask, 'warm')]:
                    if _mask.sum() > 0:
                        _sim_g = F.cosine_similarity(_h_cpu[_mask], _e_cpu[_mask]).mean().item()
                        print(f"[DIAG_GROUP] epoch={iter_idx:>3d}  {_lbl}  cosine_sim_preGCN={_sim_g:.4f}  n={_mask.sum().item()}", flush=True)
        ...
        valid_ndcg, valid_cold, valid_noncold, valid_medium, valid_warm = net.evaluate_ranking_ndcg(valid_dataloader, dataset, K=10, etype='valid')
        print(f"[VALID_GROUPS] epoch={iter_idx:>3d}  global={valid_ndcg:.4f}  cold={valid_cold:.4f}  noncold={valid_noncold:.4f}  frozen={_frozen}", flush=True)
        logging_str = (f"Epoch={iter_idx:>3d}, "
                       f"Train_BPR={r_loss.item():.4f}, MI={mi_score:.2f}, Valid_nDCG@10={valid_ndcg:.4f}, ")

        if not _frozen:
            # ── Phase 1 : checkpoint global (pas d'early stop) + détection pic non-cold ──────
            if valid_ndcg > best_valid_ndcg:
                best_valid_ndcg = valid_ndcg
                best_iter       = iter_idx
                test_ndcg, test_cold, _, test_medium, test_warm = net.evaluate_ranking_ndcg(test_dataloader, dataset, K=10, etype='test')
                best_test_ndcg  = test_ndcg
                logging_str    += f'Test_nDCG@10={test_ndcg:.4f}'
                print(f"[TEST_GROUPS_BEST] epoch={iter_idx:>3d}  global={test_ndcg:.4f}  cold={test_cold:.4f}  medium={test_medium:.4f}  warm={test_warm:.4f}", flush=True)
                checkpoint      = net.state_dict()
                checkpoint['modal_enc'] = modal_enc.state_dict()
                torch.save(checkpoint, params.model_save_path)

            # Suivi non-cold pour détecter le pic → freeze quand non-cold stagne
            if valid_noncold > _best_noncold_valid:
                _best_noncold_valid = valid_noncold
                _no_better_noncold  = 0
            else:
                _no_better_noncold += 1
                if _no_better_noncold >= params.freeze_patience:
                    # Pic non-cold atteint → geler les items non-cold
                    _noncold_mask_1d = (_diag_deg > params.cold_threshold).float().to(params.device)
                    def _freeze_noncold_hook(grad):
                        _m = _noncold_mask_1d
                        for _ in range(grad.dim() - 1):
                            _m = _m.unsqueeze(-1)
                        return grad * (1.0 - _m)
                    net.rating_encoder.item_embedding.register_hook(_freeze_noncold_hook)
                    _frozen    = True
                    _n_frozen  = int((_diag_deg > params.cold_threshold).sum().item())
                    _n_cold    = int((_diag_deg <= params.cold_threshold).sum().item())
                    print(f"[ADAPTFREEZE] epoch={iter_idx}  FREEZE déclenché — "
                          f"{_n_frozen} non-cold gelés, {_n_cold} cold libres "
                          f"(best_noncold={_best_noncold_valid:.4f}, patience={params.freeze_patience})", flush=True)
                    _best_cold_postfreeze = valid_cold
                    _no_better_cold_pf    = 0

        else:
            # ── Phase 2 : early stopping sur cold uniquement ──────────────────
            if valid_cold > _best_cold_postfreeze:
                _best_cold_postfreeze = valid_cold
                _no_better_cold_pf    = 0
                best_iter             = iter_idx
                test_ndcg, test_cold, _, test_medium, test_warm = net.evaluate_ranking_ndcg(test_dataloader, dataset, K=10, etype='test')
                best_test_ndcg = test_ndcg
                logging_str   += f'Test_nDCG@10={test_ndcg:.4f}'
                print(f"[TEST_GROUPS_POSTFREEZE] epoch={iter_idx:>3d}  global={test_ndcg:.4f}  cold={test_cold:.4f}  medium={test_medium:.4f}  warm={test_warm:.4f}", flush=True)
                checkpoint = net.state_dict()
                checkpoint['modal_enc'] = modal_enc.state_dict()
                torch.save(checkpoint, params.model_save_path)
            else:
                _no_better_cold_pf += 1
                if _no_better_cold_pf >= params.train_early_stopping_patience:
                    logger.info("Early stopping threshold reached (cold post-freeze). Stop training.")
                    break

        # checkpoint best cold (indépendant du global, toujours actif)
        if valid_cold > best_cold_valid_ndcg:
            best_cold_valid_ndcg = valid_cold
            best_cold_iter       = iter_idx
            cold_ckpt            = net.state_dict()
            cold_ckpt['modal_enc'] = modal_enc.state_dict()
            torch.save(cold_ckpt, cold_save_path)
            print(f"[COLD_CKPT] epoch={iter_idx}  best_cold_valid={valid_cold:.4f}  saved → {cold_save_path}", flush=True)
    ...
    logger.info(f'Best Cold Iter={best_cold_iter}, Best Cold Valid nDCG@10={best_cold_valid_ndcg:.4f}')
...
def test(params):
    ...
    if getattr(params, 'use_bestcold', False):
        params.model_save_path = params.model_save_path.replace('.pt', '_bestcold.pt')

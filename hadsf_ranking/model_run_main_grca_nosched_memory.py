...
class SentenceRetrival(nn.Module):
    ...
    def __init__(self, in_units, num_classes, review_embedding, sentence_embedding, dropout_rate=0.0):
        ...
        # Cross-batch memory queue (4096 phrases des batches précédents)
        _Q = 4096
        self.register_buffer('_sent_queue', torch.zeros(_Q, in_units))
        self.register_buffer('_queue_ptr',  torch.zeros(1, dtype=torch.long))
        self.register_buffer('_queue_size', torch.zeros(1, dtype=torch.long))

    def _enqueue(self, feats):
        Q = self._sent_queue.shape[0]
        n = feats.shape[0]
        ptr = int(self._queue_ptr.item())
        if ptr + n <= Q:
            self._sent_queue[ptr:ptr+n] = feats
        else:
            end = Q - ptr
            self._sent_queue[ptr:] = feats[:end]
            self._sent_queue[:n-end] = feats[end:]
        self._queue_ptr[0] = (ptr + n) % Q
        self._queue_size[0] = min(self._queue_size.item() + n, Q)
    ...
    def forward(self, graph, urf, irf, utf, itf, neg_strategy='random', n_neg=1):
        ...
        with graph.local_scope():
            if neg_strategy == 'inbatch':
                ...
                pos_score = (th * pos_feat).sum(1)               # (N,)
                # Négatifs = batch actuel + queue des batches précédents
                Q = int(self._queue_size.item())
                if Q > 0:
                    queue_feats = self._sent_queue[:Q].clone()   # (Q, 128)
                    all_neg = torch.cat([pos_feat, queue_feats], dim=0)  # (N+Q, 128)
                else:
                    all_neg = pos_feat                           # premier batch : batch only
                # score_mat[i,j] = interaction i vs phrase j (batch + queue)
                score_mat = th @ all_neg.T                       # (N, N+Q)
                # masquer la diagonale (positif courant) parmi les N premiers
                diag_mask = torch.zeros(N, all_neg.shape[0], dtype=torch.bool, device=th.device)
                diag_mask[:, :N] = torch.eye(N, dtype=torch.bool, device=th.device)
                score_mat = score_mat.masked_fill(diag_mask, float('-inf'))
                M = score_mat.shape[1]  # N + Q
                pos_exp = pos_score.unsqueeze(1).expand(N, M)
                bpr = -F.logsigmoid(pos_exp - score_mat)
                bpr = bpr.masked_fill(diag_mask, 0.0)
                loss = bpr.sum(1) / (M - 1)
                mi = loss.mean()
                # mettre à jour la queue avec les phrases du batch courant
                self._enqueue(pos_feat.detach())
                return mi, mi
            else:
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        # Même méthode que le training : phrase individuelle, négatifs = autres phrases du batch
        graph.nodes['user'].data['rf'] = urf
        graph.nodes['item'].data['rf'] = irf
        graph.nodes['user'].data['tf'] = utf
        graph.nodes['item'].data['tf'] = itf
        ...
        def _get(edges):
            rh = self.rating_linear(torch.cat([edges.src['rf'].to(dev), edges.dst['rf'].to(dev)], dim=1))
        ...
        # Score matrix : th (E,128) @ pos_feat.T (128,E) → (E,E)
        # score_mat[i,j] = score interaction i contre phrase j du batch
        score_mat  = (th @ pos_feat.T).cpu()              # (E, E)

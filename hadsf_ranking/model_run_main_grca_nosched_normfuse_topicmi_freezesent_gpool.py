...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_normfuse_topicmi_freezesent_gpool{_run_tag}.pt'
    )
...
class Net(nn.Module):
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_encoder_blocks in dataloader:
            ...
            th      = decoder_graph.edata['_th']
            pos_sid = decoder_graph.edata['sentence_id']
            E = th.shape[0]
            valid_m   = pos_sid > 0
        ...
        all_th    = torch.cat(all_th,    dim=0)
        all_pos_s = torch.cat(all_pos_s, dim=0)

        # Passe 2 : pool global
        all_pos_feat = self.topic_decoder.sentence_embedding(all_pos_s.to(device)).cpu()
        ...
        for start in range(0, N, chunk):
            ...
            scores    = (th_chunk @ pf_n.T.to(device)).cpu()
...
def train(params):
    ...
    repr_norm_history = []
    # Test A : freeze sentence_embedding au 1er déclin SR
    best_sr_ndcg    = 0.0
    best_sent_snap  = None
    sent_frozen     = False
    ...
    for iter_idx in range(1, params.epoch):
        ...
        if not sent_frozen:
            if cur_sr > best_sr_ndcg:
                best_sr_ndcg   = cur_sr
                best_sent_snap = copy.deepcopy(net.topic_decoder.sentence_embedding.state_dict())
                print(f"[BEST_SR] ep={iter_idx}  SR={cur_sr:.4f}  snapshot sentence_emb sauvegardé", flush=True)
            else:
                # 1er déclin → geler sentence_embedding, restaurer snapshot
                net.topic_decoder.sentence_embedding.load_state_dict(best_sent_snap)
                net.topic_decoder.sentence_embedding.weight.requires_grad_(False)
                sent_frozen = True
                print(f"[FREEZE_SENT] ep={iter_idx}  SR_pic={best_sr_ndcg:.4f} → sentence_embedding gelé+restauré", flush=True)

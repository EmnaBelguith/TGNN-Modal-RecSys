...
def config():
    ...
    parser.add_argument('--k_hard_neg',         type=int,   default=16,
                        help='Nombre de négatifs same-topic diff-item par edge')
    ...
    _neg_tag  = f'_topicneg_k{args.k_hard_neg}'
    _nneg_tag = ''
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_normfuse{_run_tag}.pt'
    )
...
class SentenceRetrival(nn.Module):
    ...
    def setup_topic_neg_index(self, raw_graph, item_to_sids):
        """Construit l'index topic-neg directement depuis le graphe courant.
        raw_graph    : DGL HeteroGraph avec sentence nodes ayant 'global_topic_id'
        item_to_sids : {item_gid: [sid, ...]} construit depuis les edges train du graphe
        """
        from collections import defaultdict

        # sid_to_gtid : depuis les sentence nodes du graphe courant (IDs alignés)
        sent_gtid = raw_graph.nodes['sentence'].data['global_topic_id']  # (n_sent,)
        n_sent    = raw_graph.num_nodes('sentence')
        # Padding pour les rares sids hors range (max 7)
        max_sid   = n_sent + 10
        sid_to_gtid_t = torch.zeros(max_sid, dtype=torch.long)
        sid_to_gtid_t[:n_sent] = sent_gtid
        self._sid_to_gtid = sid_to_gtid_t   # CPU

        # sid → item_gid (-1 si inconnu)
        sid_to_item_t = torch.full((max_sid,), -1, dtype=torch.long)
        for item_gid, sids in item_to_sids.items():
            for sid in sids:
                if sid < max_sid:
                    sid_to_item_t[sid] = item_gid
        self._sid_to_item = sid_to_item_t   # CPU

        # gtid → tensor de sids (depuis le graphe, IDs corrects)
        max_gtid = int(sent_gtid.max().item()) + 1
        gtid_buckets = defaultdict(list)
        for sid in range(n_sent):
            gtid = sent_gtid[sid].item()
            gtid_buckets[gtid].append(sid)
        self._gtid_pool = [
            torch.LongTensor(gtid_buckets.get(g, [0]))
            for g in range(max_gtid)
        ]

        # Stats
        n_topics  = sum(1 for g in range(max_gtid) if len(gtid_buckets.get(g, [])) > 0)
        n_covered = sum(len(v) for v in gtid_buckets.values())
        print(f"[TopicNeg] SOURCE = graphe courant ({n_sent} sentence nodes)", flush=True)
        print(f"[TopicNeg] {n_topics} topics, {n_covered} phrases couvertes / {n_sent}", flush=True)

    def forward(self, graph, urf, irf, utf, itf, item_gids=None, k_hard_neg=16):
        ...
        with graph.local_scope():
            ...
            th      = graph.edata['_th']     # (N, D)
            pos_sid = graph.edata['_pids']   # (N, max_sents)
            N = th.shape[0]
            pos_s    = self._pick_one_sentence(pos_sid)  # (N,)
            pos_feat = self.sentence_embedding(pos_s)    # (N, D)

            # Hard negatives : même topic global, item différent
            gtids = self._sid_to_gtid[pos_s.cpu()]       # (N,)
            neg_sids_list = []
            for i in range(N):
                pool = self._gtid_pool[gtids[i].item()]
                if item_gids is not None:
                    mask = self._sid_to_item[pool] != item_gids[i].cpu().item()
                    filtered = pool[mask]
                    if len(filtered) == 0:
                        filtered = pool   # fallback sans filtre item
                else:
                    filtered = pool
                K = k_hard_neg
                if len(filtered) >= K:
                    idx = torch.randperm(len(filtered))[:K]
                else:
                    idx = torch.randint(len(filtered), (K,))
                neg_sids_list.append(filtered[idx])
            neg_sids = torch.stack(neg_sids_list).to(th.device)   # (N, K)
            neg_feat = self.sentence_embedding(neg_sids)           # (N, K, D)

            # InfoNCE : pos à index 0, K hard-negs ensuite
            # F.normalize → on s'intéresse uniquement à la direction
            th_n  = F.normalize(th,       dim=-1)         # (N, D)
            pf_n  = F.normalize(pos_feat, dim=-1)         # (N, D)
            nf_n  = F.normalize(neg_feat, dim=-1)         # (N, K, D)
            pos_score  = (th_n * pf_n).sum(-1, keepdim=True)           # (N, 1)
            neg_scores = torch.einsum('bd,bkd->bk', th_n, nf_n)        # (N, K)
            logits = torch.cat([pos_score, neg_scores], dim=1) / self.mi_temp  # (N, K+1)
            labels = torch.zeros(N, dtype=torch.long, device=th.device)
            mi = F.cross_entropy(logits, labels)
            return mi, mi
...
class Net(nn.Module):
    ...
    def __init__(self, review_embedding, sentence_embedding, params):
        ...
        self.k_hard_neg = params.k_hard_neg
    ...
    def calc_loss(self,
        ...
        src_e, dst_e = pos_graph.edges()
        item_gids_e  = pos_graph.dstdata['_ID'][dst_e]   # global item IDs par edge
        ed_mi, ranking_loss = self.topic_decoder(
            pos_graph, urf, irf, combined_u, combined_i,
            item_gids=item_gids_e, k_hard_neg=self.k_hard_neg
        )
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_encoder_blocks in dataloader:
            ...
            self.topic_decoder._sr_formula = getattr(self, '_sr_formula', 'topic_only')
            ranking_scores, valid_idx = self.topic_decoder.get_ranking_scores(
                decoder_graph, utf, itf, urf, irf, topk
            )
            scores_list.append(ranking_scores)
            for local_i, orig_i in enumerate(valid_idx.tolist()):
                rating = ratings[orig_i]
                group = int(rating)
                for metric, values in ranking_scores.items():
                    if local_i < len(values):
                        group_scores[group][metric].append(values[local_i])
...
def train(params):
    ...
    # ── Index topic-neg : sid→gtid, gtid→sids, sid→item ──────────────────
    from collections import defaultdict
    item_to_sids = defaultdict(list)
    _train_g   = dataset.graph['train']
    _, _dst    = _train_g.edges()                        # item local IDs (= global IDs)
    _sids_all  = _train_g.edata['sentence_id']           # (n_edges, max_sents)
    for idx in range(len(_dst)):
        item_gid = _dst[idx].item()
        sids = [s.item() for s in _sids_all[idx] if s > 0]
        item_to_sids[item_gid].extend(sids)
    net.topic_decoder.setup_topic_neg_index(dataset.graph, dict(item_to_sids))
    # ─────────────────────────────────────────────────────────────────────
    ...
    print(f"[LR] No scheduler — LR fixe={params.train_lr}", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            _eff_lambda_mi = params.lambda_mi * min(1.0, iter_idx / params.lambda_mi_warmup) if params.lambda_mi_warmup > 0 else params.lambda_mi

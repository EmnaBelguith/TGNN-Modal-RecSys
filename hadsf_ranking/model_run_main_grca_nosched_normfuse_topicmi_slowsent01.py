...
def config():
    ...
    parser.add_argument('--sent_lr_scale',               type=float, default=0.1,
                        help='Facteur multiplicatif du LR pour sentence_embedding (défaut 0.1)')
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}_logdeg_normfuse_topicmi_slowsent01{_run_tag}.pt'
    )
...
class Net(nn.Module):
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        # SR eval per-batch — pool = phrases du mini-batch courant (pas pool global)

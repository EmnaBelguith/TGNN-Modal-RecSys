...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_normfuse_topicmi_freezetopic{_run_tag}.pt'
    )
...
def train(params):
    ...
    repr_norm_history = []
    # Test B : freeze topic_encoder + topic_linear au 1er déclin SR
    best_sr_ndcg    = 0.0
    best_topic_snap = None
    topic_frozen    = False
    ...
    for iter_idx in range(1, params.epoch):
        ...
        net.eval()
        sr_scores = net.evaluate_sentence_ranking(test_dataloader, graph, topic_sampler, etype='test')
        logger.info('Test - ' + format_dict_to_str(sr_scores))
        cur_sr = sr_scores.get('nDCG', 0.0)

        if not topic_frozen:
            if cur_sr > best_sr_ndcg:
                best_sr_ndcg    = cur_sr
                best_topic_snap = {
                ...
                print(f"[BEST_SR] ep={iter_idx}  SR={cur_sr:.4f}  snapshot topic_encoder+topic_linear sauvegardé", flush=True)
            else:
                # 1er déclin → restaurer + geler topic_encoder + topic_linear
                net.topic_encoder.load_state_dict(best_topic_snap['topic_encoder'])
                net.topic_decoder.topic_linear.load_state_dict(best_topic_snap['topic_linear'])
                for p in net.topic_encoder.parameters():
                    p.requires_grad_(False)
                for p in net.topic_decoder.topic_linear.parameters():
                    p.requires_grad_(False)
                topic_frozen = True
                print(f"[FREEZE_TOPIC] ep={iter_idx}  SR_pic={best_sr_ndcg:.4f} → topic_encoder+topic_linear gelés+restaurés", flush=True)

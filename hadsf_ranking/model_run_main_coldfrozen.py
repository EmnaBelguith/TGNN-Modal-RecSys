...
def config():
    ...
    parser.add_argument('--cold_freeze_max',   type=int,   default=10,
                        help='Items avec degré <= cold_freeze_max ont leur gradient item_embedding gelé (0=désactivé)')
    ...
    _frozen_tag = f'_coldfrozen{args.cold_freeze_max}' if args.cold_freeze_max > 0 else ''
    args.model_save_path = (
        f'model_save/{args.dataset_name}/{args.model_short_name}'
        f'_layers_{args.num_layers}_seed{args.seed}'
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}{_frozen_tag}{_run_tag}.pt'
    )
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        # alpha = degré normalisé directement — cold→0, warm→1, sans MLP
        ...
        if self.training:
            ...
            # diagnostic cold/warm direct
            if getattr(self, '_gate_step', 0) % 100 == 0:
                ...
                if _cold_mask.any() and _warm_mask.any():
                    ...
                    print(f"[DEGALPHA] cold={_a[_cold_mask].mean():.3f}  warm={_a[_warm_mask].mean():.3f}", flush=True)
        ...
        item_outputs = item_outputs + (1 - alpha) * _item_modal_out  # résiduel direct
...
def train(params):
    ...
    # cold items dont gradients item_embedding sont gelés pendant BPR
    if params.cold_freeze_max > 0:
        _cold_freeze_ids = torch.where(
            (_deg_tensor_train >= 5) & (_deg_tensor_train <= params.cold_freeze_max)
        )[0].to(params.device)
        print(f"[COLDFROZEN] {len(_cold_freeze_ids)} items gelés (degré 5–{params.cold_freeze_max})", flush=True)
    else:
        _cold_freeze_ids = None
        print("[COLDFROZEN] désactivé (cold_freeze_max=0)", flush=True)
    ...
    with torch.no_grad():
        ...
        print(f"[DEGALPHA] alpha = log1p(degree)/log1p(max) — pas de gate MLP", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            # gel des gradients pour les cold items — ils restent ancrés à h_modal init
            if _cold_freeze_ids is not None and net.rating_encoder.item_embedding.grad is not None:
                net.rating_encoder.item_embedding.grad[_cold_freeze_ids] = 0.0
            optimizer.step()

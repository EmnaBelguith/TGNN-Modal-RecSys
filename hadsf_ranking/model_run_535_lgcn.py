...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_lgcn{_run_tag}.pt'
    )
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def __init__(self, rating_values, review_embedding, user_size, item_size, msg_units, num_layers, aggregate='sum', dropout_rate=0.0, minmax_alpha=False, no_review=False):
...
class Net(nn.Module):
    ...
    def calc_loss(self,
        ...
        # LightGCN-style : 1 négatif random catalogue (pas vu par le user)
        train_seen_local = getattr(self, '_train_seen', None)
        item_size_local  = self.rating_encoder.item_embedding.shape[0]
        user_global_ids  = pos_graph.srcdata['_ID'][src_pos].cpu().tolist()
        bpr_terms = []
        for _ in range(self.n_neg):
            neg_ids = []
            for uid in user_global_ids:
                seen = train_seen_local[uid] if train_seen_local else set()
                for _ in range(200):
                    neg = random.randint(0, item_size_local - 1)
                    if neg not in seen:
                        neg_ids.append(neg)
                        break
                else:
                    neg_ids.append(random.randint(0, item_size_local - 1))
            neg_tensor = torch.tensor(neg_ids, device=i_pos_emb.device)
            # Approximation : item_embedding brut → ifc(GELU(.)) sans GCN complet
            i_neg_raw = self.rating_encoder.item_embedding[neg_tensor]
            i_neg_emb = self.rating_encoder.ifc(self.rating_encoder.agg_act(i_neg_raw))
    ...
    @torch.no_grad()
    def evaluate_full_ranking(self, train_dataloader, eval_dataloader, dataset,
                               ks=(5, 10, 20), relevance_threshold=1, etype='test'):
        import math
        device = self.review_embedding.weight.device
        self.eval()
        graph = dataset.graph
        ...
        # Phase 1 : embeddings de TOUS les items via train_dataloader
        item_emb = {}
        for input_nodes, pos_graph, _, blocks in train_dataloader:
            inp = {k: v.to(device) for k, v in input_nodes.items()}
            blk = [b.to(device) for b in blocks]
            _, irf = self.rating_encoder(inp, blk)
            g_iids = blocks[-1].dstdata['_ID']['item'].cpu().tolist()
            for li, gi in enumerate(g_iids):
                if gi not in item_emb: item_emb[gi] = irf[li].cpu()

        # Phase 2 : embeddings users + items test
        user_emb = {}
        test_items = defaultdict(dict)
        for input_nodes, pos_graph, _, blocks in eval_dataloader:
            inp = {k: v.to(device) for k, v in input_nodes.items()}
            pg  = pos_graph[etype].to(device)
            blk = [b.to(device) for b in blocks]
            urf, irf = self.rating_encoder(inp, blk)
            ...
            for uid, iid, r in zip(pg.srcdata['_ID'][src_idx].cpu().tolist(),
                ...
                test_items[uid][iid] = r

        # Phase 3 : full ranking
        rating_linear = self.topic_decoder.rating_linear.to('cpu')
        item_scorer   = self.topic_decoder.item_scorer.to('cpu')
        all_iids  = np.array(sorted(item_emb.keys()))
        all_embs  = torch.stack([item_emb[i] for i in all_iids])

        results     = {k: {'ndcg': [], 'recall': [], 'hr': [], 'precision': []} for k in ks}
        grp_results = {g: {k: {'ndcg': [], 'recall': [], 'hr': [], 'precision': []} for k in ks}
                       for g in ('cold', 'medium', 'warm', 'non_cold')}
        n_eval = 0

        for uid, items in test_items.items():
            if uid not in user_emb: continue
            relevant = {iid for iid, r in items.items() if r >= relevance_threshold}
            if not relevant: continue
            u_rep  = user_emb[uid].unsqueeze(0).expand(len(all_iids), -1)
            scores = item_scorer(rating_linear(torch.cat([u_rep, all_embs], dim=1))).squeeze(-1)
            mask   = torch.tensor([all_iids[i] in train_seen[uid] for i in range(len(all_iids))])
            scores[mask] = float('-inf')
            _, ridx    = scores.sort(descending=True)
            ranked_ids = all_iids[ridx.numpy()]
            ...
            for k in ks:
                ...
                prec_v = hits/k
                results[k]['ndcg'].append(ndcg_v); results[k]['recall'].append(rec_v)
                results[k]['hr'].append(hr_v);     results[k]['precision'].append(prec_v)
                if group:
                    grp_results[group][k]['ndcg'].append(ndcg_v); grp_results[group][k]['recall'].append(rec_v)
                    grp_results[group][k]['hr'].append(hr_v);     grp_results[group][k]['precision'].append(prec_v)
                    if group in ('medium', 'warm'):
                        grp_results['non_cold'][k]['ndcg'].append(ndcg_v); grp_results['non_cold'][k]['recall'].append(rec_v)
                        grp_results['non_cold'][k]['hr'].append(hr_v);     grp_results['non_cold'][k]['precision'].append(prec_v)

        self.topic_decoder.rating_linear.to(device); self.topic_decoder.item_scorer.to(device)
        print(f"  [full_ranking] {n_eval} users, {len(all_iids)} items", flush=True)
        global_out = {k: {m: float(np.mean(v)) if v else 0.0 for m, v in mt.items()} for k, mt in results.items()}
        grp_out    = {g: {k: {m: float(np.mean(v)) if v else 0.0 for m, v in mt.items()}
                          for k, mt in gmt.items()} for g, gmt in grp_results.items()}
...
def train(params):
    ...
    # LightGCN : construire train_seen pour les négatifs random catalogue
    _train_seen = defaultdict(set)
    _tu, _ti = graph['train'].edges()
    for _u, _i in zip(_tu.tolist(), _ti.tolist()):
        _train_seen[_u].add(_i)
    net._train_seen = _train_seen  # injecté pour le random neg sampling LightGCN
    ...
    for iter_idx in range(1, params.epoch):
        ...
        _vres, _ = net.evaluate_full_ranking(train_dataloader, valid_dataloader, dataset, ks=(10,), etype='valid')
        valid_ndcg = _vres[10]['ndcg']
        ...
        if valid_ndcg > best_valid_ndcg:
            ...
            no_better_valid = 0
            best_iter       = iter_idx
            _tres, _ = net.evaluate_full_ranking(train_dataloader, test_dataloader, dataset, ks=(10,), etype='test')
            test_ndcg      = _tres[10]['ndcg']

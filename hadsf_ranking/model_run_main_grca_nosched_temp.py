...
class SentenceRetrival(nn.Module):
    ...
    def __init__(self, in_units, num_classes, review_embedding, sentence_embedding, dropout_rate=0.0):
        ...
        self.tau = nn.Parameter(torch.tensor(0.07))  # température apprise
    ...
    def forward(self, graph, urf, irf, utf, itf, neg_strategy='random', n_neg=1):
        ...
        with graph.local_scope():
            if neg_strategy == 'inbatch':
                ...
                tau = self.tau.abs().clamp(min=1e-4)
                pos_score = (th * pos_feat).sum(1) / tau      # (N,)
                # score_mat[i,j] = score de l'interaction i contre la phrase j du batch
                score_mat = (th @ pos_feat.T) / tau           # (N, N)

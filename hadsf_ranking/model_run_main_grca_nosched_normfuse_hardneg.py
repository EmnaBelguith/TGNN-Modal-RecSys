...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_normfuse_hardneg{_run_tag}.pt'
    )
...
class SentenceRetrival(nn.Module):
    ...
    def __init__(self, in_units, num_classes, review_embedding, sentence_embedding, dropout_rate=0.0, mi_temp=0.1):
        ...
        self._item_train_sids = None   # set via set_item_train_sids() avant le training

    def set_item_train_sids(self, item_train_sids):
        # item_train_sids : LongTensor [n_items, K_max] — phrases training par item
        self._item_train_sids = item_train_sids
    ...
    def forward(self, graph, urf, irf, utf, itf, neg_strategy='random', n_neg=1):
        ...
        with graph.local_scope():
            if neg_strategy == 'inbatch':
                ...
                item_gids = graph.edata['_iid']                # (N,) global item ids
                N = th.shape[0]
                pos_s    = self._pick_one_sentence(pos_sid)    # (N,) phrase positive
                pos_feat = self.sentence_embedding(pos_s)      # (N, D)

                if self._item_train_sids is not None:
                    # ── Same-item hard negatives ──────────────────────────────
                    # neg_pool[i] = toutes les phrases training de l'item i
                    neg_pool  = self._item_train_sids[item_gids]  # (N, K_max)
                    # Choisir K phrases valides (>0) au hasard dans le pool
                    valid_mask = neg_pool > 0
                    rand_w     = valid_mask.float() * torch.rand(neg_pool.shape, device=neg_pool.device)
                    K = min(16, neg_pool.shape[1])
                    _, top_idx = torch.topk(rand_w, k=K, dim=1)   # (N, K)
                    neg_sids   = neg_pool.gather(1, top_idx)       # (N, K)
                    neg_feat   = self.sentence_embedding(neg_sids) # (N, K, D)
                    # InfoNCE : positif en index 0, K négatifs same-item ensuite
                    th_n       = F.normalize(th,       dim=-1)
                    pf_n       = F.normalize(pos_feat, dim=-1)
                    nf_n       = F.normalize(neg_feat, dim=-1)
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        # normfuse : F.normalize(utf)+F.normalize(urf) — même que le training
        combined_u = F.normalize(utf, dim=-1) + F.normalize(urf, dim=-1)
        combined_i = F.normalize(itf, dim=-1) + F.normalize(irf, dim=-1)
        graph.nodes['user'].data['tf'] = combined_u
        graph.nodes['item'].data['tf'] = combined_i
        ...
        # score_mat[i,j] = score interaction i contre phrase j du batch
        th_n       = F.normalize(th,       dim=-1)
        pf_n       = F.normalize(pos_feat, dim=-1)
        score_mat  = (th_n @ pf_n.T).cpu()                # (E, E)
        _, ranked  = torch.sort(score_mat, dim=1, descending=True)  # (E, E)
        topk_idx   = ranked[:, :topk].numpy()             # (E, topk) — indices dans le batch
        topk_sids  = pos_s_np[topk_idx]                   # (E, topk) — sentence IDs
        true_sids  = pos_s_np.reshape(-1, 1)              # (E, 1)
        return calc_ranking_metrics(topk_sids, true_sids)
...
def train(params):
    ...
    # ── Construire item_train_sids : phrases training par item ──────────────
    from collections import defaultdict
    import pickle as _pkl
    _dir = f'/home/infres/belguith/PFE/HADSF_test/checkpoint/{params.dataset_name}/BERT-Whitening'
    with open(f'{_dir}/ui_to_sentence_id.pkl', 'rb') as _f:
        _ui_to_sid = _pkl.load(_f)
    _item_to_sids = defaultdict(list)
    _train_g = dataset.graph[('user', 'train', 'item')]
    _train_u, _train_i = _train_g.edges()
    for _idx in range(_train_u.shape[0]):
        _u = _train_u[_idx].item()
        _i = _train_i[_idx].item()
        if (_u, _i) in _ui_to_sid:
            _item_to_sids[_i].extend(_ui_to_sid[(_u, _i)])
    _K_max = 32
    _item_train_sids = torch.zeros(dataset.item_size, _K_max, dtype=torch.long)
    for _i, _sids in _item_to_sids.items():
        _sids_t = torch.LongTensor(list(set(_sids))[:_K_max])
        _item_train_sids[_i, :len(_sids_t)] = _sids_t
    net.topic_decoder.set_item_train_sids(_item_train_sids.to(params.device))
    _n_items_with_negs = sum(1 for v in _item_to_sids.values() if len(v) > 0)
    print(f"[HARDNEG] item_train_sids construit — {_n_items_with_negs}/{dataset.item_size} items avec phrases training, K_max={_K_max}", flush=True)

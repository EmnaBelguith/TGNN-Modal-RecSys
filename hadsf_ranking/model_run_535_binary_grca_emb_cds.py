...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}{_rinit_tag}{_bprw_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_grca_emb_cds{_run_tag}.pt'
    )
    args.resume_ckpt_path = args.model_save_path.replace('.pt', '_resume.pt')
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_encoder_blocks in dataloader:
            ...
            if has_topic is not None:
                _ps = pos_s.cpu()
                tmask = has_topic[_ps.clamp(max=len(has_topic)-1)] & (_ps < len(has_topic))
...
def train(params):
    ...
    _n_sents = net.topic_decoder.sentence_embedding.num_embeddings
    _has_topic = torch.zeros(_n_sents, dtype=torch.bool)
    for _sid in _sid_to_topic:
        _has_topic[_sid] = True
    net.topic_decoder._has_topic = _has_topic
    print(f"[SR_FILTER] {_has_topic.sum().item():,} sids avec topic sur {_n_sents:,} total", flush=True)
    ...
    import pandas as pd
    # CDs_interactions_reviews.csv : IDs alignés sur CDs_reviews_item2id.json
    ...
    print("[INIT] item_degree_tensor injected (CDs_interactions_reviews.csv)", flush=True)
    ...
    # ── Reprise depuis checkpoint périodique ────────────────────────────────
    if os.path.exists(params.resume_ckpt_path):
        print(f"[RESUME] Chargement checkpoint: {params.resume_ckpt_path}", flush=True)
        _ckpt = torch.load(params.resume_ckpt_path, map_location=params.device)
        net.load_state_dict(_ckpt['net'])
        modal_enc.load_state_dict(_ckpt['modal_enc'])
        optimizer.load_state_dict(_ckpt['optimizer'])
        start_epoch       = _ckpt['epoch'] + 1
        best_valid_ndcg   = _ckpt['best_valid_ndcg']
        best_test_ndcg    = _ckpt['best_test_ndcg']
        no_better_valid   = _ckpt['no_better_valid']
        best_iter         = _ckpt['best_iter']
        repr_norm_history = _ckpt['repr_norm_history']
        _mi_history       = _ckpt['mi_history']
        print(f"[RESUME] Reprise à epoch {start_epoch}, best_valid={best_valid_ndcg:.4f}, best_iter={best_iter}", flush=True)
    else:
    ...
    for iter_idx in range(start_epoch, params.epoch):
        net.rating_encoder.reset_epoch_diag()
        net.train()
        _epoch_mi_sum   = 0.0
        _epoch_mi_count = 0
        ...
        # ── Checkpoint périodique (reprise) ────────────────────────────────
        if iter_idx % 200 == 0:
            torch.save({
                'net':              net.state_dict(),
                'modal_enc':        modal_enc.state_dict(),
                'optimizer':        optimizer.state_dict(),
                'epoch':            iter_idx,
                'best_valid_ndcg':  best_valid_ndcg,
                'best_test_ndcg':   best_test_ndcg,
                'no_better_valid':  no_better_valid,
                'best_iter':        best_iter,
                'repr_norm_history': repr_norm_history,
                'mi_history':       _mi_history,
            }, params.resume_ckpt_path)
            print(f"[CHECKPOINT] Resume checkpoint sauvegardé — epoch {iter_idx}", flush=True)

...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        _modal_adapted = self.modal_adapter(_item_modal_out)
        item_outputs = alpha * item_outputs + (1 - alpha) * _modal_adapted
...
class Net(nn.Module):
    ...
    def _get_item_emb_global(self, global_iids):
        ...
        _ma       = self.rating_encoder.modal_adapter(modal)
        emb       = alpha * collab + (1 - alpha) * _ma

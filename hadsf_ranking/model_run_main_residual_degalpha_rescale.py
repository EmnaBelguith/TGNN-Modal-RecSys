...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_degalpha_floss_rescale{_run_tag}.pt'
    )
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def forward(self, input_nodes, encoder_blocks):
        ...
        # rescale h_modal à la norme de collab → alpha contrôle vraiment le mélange
        _collab_norm = item_outputs.norm(dim=-1, keepdim=True).detach()
        _modal_norm  = _item_modal_out.norm(dim=-1, keepdim=True).detach().clamp(min=1e-8)
        _item_modal_rescaled = _item_modal_out * (_collab_norm / _modal_norm)
        item_outputs = item_outputs + (1 - alpha) * _item_modal_rescaled
...
class Net(nn.Module):
    ...
    def _get_item_emb_global(self, global_iids):
        ...
        _collab_norm = collab.norm(dim=-1, keepdim=True).clamp(min=1e-8)
        _modal_norm  = modal.norm(dim=-1, keepdim=True).clamp(min=1e-8)
        modal_rescaled = modal * (_collab_norm / _modal_norm)
        emb    = collab + (1 - alpha) * modal_rescaled

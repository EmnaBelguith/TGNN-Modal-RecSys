...
def config():
    ...
    parser.add_argument('--rand_init',         action='store_true', default=False,
                        help='Init item_embedding direction aléatoire (user_norm scale) — GRCA reste actif')
    ...
    parser.add_argument('--floss_exp', type=float, default=1.0,
                        help='Exposant sur alpha_f : 1=linéaire, 2=alpha², >2 réduit encore plus cold')
    ...
    _floss_tag = f'_f{args.lambda_f}'       if args.lambda_f > 0          else '_nofloss'
    _fexp_tag  = f'_fexp{args.floss_exp}'   if getattr(args, 'floss_exp', 1.0) != 1.0 else ''
    args.model_save_path = (
        f'model_save/{args.dataset_name}/{args.model_short_name}'
        f'_layers_{args.num_layers}_seed{args.seed}'
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}{_rinit_tag}{_floss_tag}{_fexp_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_baby{_run_tag}.pt'
    )
    ...
    if not params.no_modal:
        with torch.no_grad():
            ...
            if params.rand_init:
                ...
                print(f"[INIT] item_embedding ← random direction (user_norm {_target_norm:.3f}) — GRCA actif", flush=True)
            else:
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            if not params.no_modal and _n_f > 1:
                ...
                f_loss = net.compute_fusion_loss(
                    ...
                    lambda_f=params.lambda_f, weights=_alpha_f ** params.floss_exp,

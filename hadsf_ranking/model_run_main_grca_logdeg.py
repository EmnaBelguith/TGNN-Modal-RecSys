...
def config():
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg{_run_tag}.pt'
    )
...
class SentenceRetrival(nn.Module):
    ...
    def calc_sentence_ranking(self, edges):
        ...
        th     = F.normalize(th_val + rh, dim=-1)
        pos_sid = edges.data['sentence_id']                  # (N, max_s)
        N       = pos_sid.shape[0]

        # pos = 1 phrase exacte (première phrase non-nulle de la review)
        pos_single = pos_sid[:, 0].clamp(min=1)              # (N,)
        pos_review = F.normalize(
            self.sentence_embedding(pos_single), dim=-1)     # (N, dim)
        pos_score  = (th * pos_review).sum(1)

        # neg = phrases aléatoires du vocab
        n_neg = getattr(self, '_n_neg', 1)
        losses = []
        for _ in range(n_neg):
            neg_single = torch.randint(1, self.sentence_embedding.weight.shape[0],
                                       (N,), device=pos_sid.device)
            neg_review = F.normalize(
                self.sentence_embedding(neg_single), dim=-1)
        ...
        return {'mi_score':          torch.stack(losses, dim=0).mean(0),
                ...
                '_rh':               rh,
                '_th_topic':         th_val,
                '_pos_review':       pos_review,
                '_neg_score':        neg_score.detach(),
                '_pos_mean_sim':     pos_score.detach(),
                '_pos_max_sent_sim': pos_score.detach(),
                '_pos_avg_sent_sim': pos_score.detach()}
        ...
        with graph.local_scope():
            if neg_strategy == 'inbatch':
                def _store(edges):
                    rh     = self.rating_linear(torch.cat([edges.src['rf'], edges.dst['rf']], dim=1))
                    th_val = self.topic_linear(torch.cat([edges.src['tf'], edges.dst['tf']], dim=1))
                    # th_eval = eval path = topic_linear seul sur cat[tf,tf] = cat[utf+urf, itf+irf]
                    # identique à get_ranking_scores — tf contient déjà utf+urf (passé par calc_loss)
                    return {'_th': th_val + rh, '_rh': rh, '_th_topic': th_val, '_pids': edges.data['sentence_id']}
                graph.apply_edges(_store)
                th       = F.normalize(graph.edata['_th'],       dim=-1)
                rh_all   = graph.edata['_rh']
                th_topic = graph.edata['_th_topic']
                pos_sid  = graph.edata['_pids']
                pos_review = F.normalize(self.get_review_feature(pos_sid), dim=-1)
                pos_score = (th * pos_review).sum(1)
                N = th.shape[0]
                ...
                # Diagnostics toutes les 200 steps
                self._mi_step = getattr(self, '_mi_step', 0) + 1
                if self._mi_step % 200 == 1:
                    with torch.no_grad():
                        rh_n    = rh_all.norm(dim=-1).mean().item()
                        topic_n = th_topic.norm(dim=-1).mean().item()

                        # --- pos vs inter-item (in-batch) ---
                        pos_mean  = pos_score.mean().item()
                        _sm = score_mat.masked_fill(diag, float('nan'))
                        inter_neg = _sm[~_sm.isnan()].mean().item()
                        inter_margin = pos_mean - inter_neg

                        # --- intra-item : négatif = autre phrase du MÊME item ---
                        # candidate_sentence_id contient toutes les phrases de l'item
                        cand = graph.edata.get('cand_sid', None)
                        if cand is None:
                            # récupérer depuis les noeuds item du graphe
                            item_ids = graph.edges()[1]  # dst = item
                            cand = graph.nodes['item'].data['candidate_sentence_id'][item_ids]
                        # pour chaque edge, tirer une phrase candidate != pos_sid
                        flat_pos = pos_sid[:, 0]          # première phrase du pos_sid
                        cand_len = (cand > 0).sum(dim=1)  # nb phrases dispo par item
                        rnd_idx  = (torch.rand(N, device=th.device) * cand_len.float()).long().clamp(max=cand.shape[1]-1)
                        intra_sid = cand.gather(1, rnd_idx.unsqueeze(1)).squeeze(1)
                        # éviter de tomber sur la même phrase
                        same = (intra_sid == flat_pos)
                        intra_sid[same] = (intra_sid[same] % (self.sentence_embedding.num_embeddings - 1)) + 1
                        intra_rev  = F.normalize(self.get_review_feature(intra_sid.unsqueeze(1)), dim=-1)
                        intra_neg  = (th * intra_rev).sum(1).mean().item()
                        intra_margin = pos_mean - intra_neg

                        # --- similarité moyenne entre phrases du même item (difficulté tâche) ---
                        # cos moyen entre pos_review et intra_rev = à quel point les phrases se ressemblent
                        sent_sim = (pos_review * intra_rev).sum(1).mean().item()

                        print(
                            f"[MI_DIAG_908] step={self._mi_step:>5d} | "
                            f"rh_norm={rh_n:.3f}  topic_norm={topic_n:.3f} | "
                            f"pos={pos_mean:.3f} | "
                            f"inter_neg={inter_neg:.3f}  inter_margin={inter_margin:.3f} | "
                            f"intra_neg={intra_neg:.3f}  intra_margin={intra_margin:.3f} | "
                            f"sent_sim(intra)={sent_sim:.3f}",
                            flush=True
                        )
                return mi, mi
            else:
                ...
                if self._mi_step % 200 == 1:
                    with torch.no_grad():
                        rh_all     = graph.edata['_rh']
                        th_topic   = graph.edata['_th_topic']
                        pos_review = graph.edata['_pos_review']  # déjà F.normalized
                        neg_score_all = graph.edata['_neg_score']
                        th         = th_topic + rh_all
                        rh_n       = rh_all.norm(dim=-1).mean().item()
                        topic_n    = th_topic.norm(dim=-1).mean().item()
                        th_n_      = th.norm(dim=-1).mean().item()
                        th_n       = F.normalize(th,       dim=-1)
                        th_eval_n  = F.normalize(th_topic, dim=-1)
                        pos_mean       = F.cosine_similarity(th_n, pos_review, dim=-1).mean().item()
                        neg_mean       = neg_score_all.mean().item()
                        margin         = pos_mean - neg_mean
                        cos_eval_rev   = F.cosine_similarity(th_eval_n, pos_review, dim=-1).mean().item()
                        cos_train_eval = F.cosine_similarity(th_n, th_eval_n, dim=-1).mean().item()
                        # clé : mean_review vs phrases individuelles
                        pos_single_cos = graph.edata['_pos_mean_sim'].mean().item()
                        print(
                            f"[MI_DIAG_908] step={self._mi_step:>5d} | "
                            f"rh_norm={rh_n:.3f}  topic_norm={topic_n:.3f}  th_norm={th_n_:.3f} | "
                            f"pos={pos_mean:.3f}  neg={neg_mean:.3f}  margin={margin:.3f} | "
                            f"cos(th,pos_single)={pos_single_cos:.3f}",
                            flush=True
                        )
    ...
    def measure_sim(self, interaction_feat, sid_list):
        ...
        sent_feat = F.normalize(self.sentence_embedding(sid_list), dim=-1)
    ...
    def get_ranking_scores(self, graph, user_feat, item_feat, topk=5,
                           urf=None, irf=None, seed=0):
        """Évalue SR avec exactement le même protocole que MI :
           pos = 1 phrase aléatoire de la review,
           pool = toutes les phrases candidates du même item (~39).
        """
        ...
        if urf is not None:
            ...
            graph.nodes['item'].data['rf'] = irf
        def _get(edges):
            th_val = self.topic_linear(torch.cat([edges.src['th'], edges.dst['th']], dim=1))
            if 'rf' in edges.src:
                rh = self.rating_linear(torch.cat([edges.src['rf'], edges.dst['rf']], dim=1))
                h  = F.normalize(th_val + rh, dim=-1)
            else:
                h  = F.normalize(th_val, dim=-1)
            return {'th': h}
        graph.apply_edges(_get)
        h          = graph.edata['th']                              # (E, dim)
        true_sents = graph.edata['sentence_id']                     # (E, max_s)
        _, dst     = graph.edges()
        all_cands  = graph.nodes['item'].data['candidate_sentence_id'][dst]  # (E, max_cand)

        vocab_size = self.sentence_embedding.num_embeddings
        N     = h.shape[0]
        n_neg = 10  # 1 pos + 10 random vocab neg

        # pos = 1 phrase exacte (première phrase de la review — même que MI)
        pos_single = true_sents[:, 0].clamp(min=1).to(h.device)
        pos_feat   = F.normalize(self.sentence_embedding(pos_single), dim=-1)  # (N, dim)

        # 10 négatifs aléatoires du vocab (même distribution que MI)
        neg_sids  = torch.randint(1, vocab_size, (N, n_neg), device=h.device)
        neg_feats = F.normalize(self.sentence_embedding(neg_sids), dim=-1)     # (N, 10, dim)

        # scores
        pos_score  = (h * pos_feat).sum(1, keepdim=True)              # (N, 1)
        neg_scores = torch.einsum('bd,bkd->bk', h, neg_feats)         # (N, 10)
        all_scores = torch.cat([pos_score, neg_scores], dim=1)         # (N, 11)

        # rang du positif parmi les 11 (rang 1-based)
        rank_of_pos = (all_scores >= pos_score).sum(dim=1).float()
        ndcg = torch.where(rank_of_pos <= topk,
                           1.0 / torch.log2(rank_of_pos + 1),
                           torch.zeros_like(rank_of_pos))

        result = {
            'nDCG': ndcg.cpu().tolist(),
            'Pre':  (rank_of_pos <= 1).float().cpu().tolist(),
            'Rec':  (rank_of_pos <= 5).float().cpu().tolist(),
            'F1':   (rank_of_pos <= 10).float().cpu().tolist(),
        }
        rank_list = None
...
class Net(nn.Module):
    ...
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5):
        ...
        # diagnostics SR_recoverable
        ndcg_all, ndcg_rec = [], []
        ranks_rec = []
        hit1_rec, hit5_rec, hit10_rec = [], [], []
        n_impossible = 0
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_encoder_blocks in dataloader:
            ...
            true_sents = decoder_graph.edata['sentence_id'].cpu()          # (E, max_s)
            cand_sents = decoder_graph.nodes['item'].data['candidate_sentence_id']  # sur item nodes
            # récupérer cand par edge
            _, dst = decoder_graph.edges()
            cand_per_edge = cand_sents[dst].cpu()                          # (E, max_cand)
            ...
            # --- diagnostics récupérables ---
            # score toutes les phrases candidates pour avoir le rang exact
            h_all = decoder_graph.edata['th'].cpu()  # déjà calculé dans get_ranking_scores
            sent_emb = self.topic_decoder.sentence_embedding.weight.detach().cpu()

            for i in range(true_sents.shape[0]):
                pos_ids  = set(true_sents[i][true_sents[i] > 0].tolist())
                pool_ids = cand_per_edge[i][cand_per_edge[i] > 0].tolist()
                pool_set = set(pool_ids)

                ndcg_all.append(ranking_scores['nDCG'][i])

                if not (pos_ids & pool_set):
                    n_impossible += 1
                    continue

                # rang exact : score h[i] vs toutes les phrases du pool
                h_i    = h_all[i]                                   # (128,)
                pool_t = torch.tensor(pool_ids, dtype=torch.long)
                sents  = torch.nn.functional.normalize(sent_emb[pool_t], dim=-1)  # (P, 128)
                scores = (sents @ h_i).tolist()                     # (P,)
                ranked = sorted(zip(scores, pool_ids), reverse=True)

                # rang de la première phrase positive
                rank = next((r+1 for r, (_, sid) in enumerate(ranked) if sid in pos_ids), len(pool_ids)+1)
                ranks_rec.append(rank)
                ndcg_rec.append(ranking_scores['nDCG'][i])
                hit1_rec.append(1 if rank <= 1 else 0)
                hit5_rec.append(1 if rank <= 5 else 0)
                hit10_rec.append(1 if rank <= 10 else 0)
        ...
        n_total = len(ndcg_all)
        n_rec   = len(ndcg_rec)
        print(f"[SR_DIAG] total={n_total}  impossible={n_impossible}({100*n_impossible/n_total:.1f}%)  "
              f"recoverable={n_rec}({100*n_rec/n_total:.1f}%)")
        if n_rec > 0:
            print(f"[SR_DIAG] nDCG_global={np.mean(ndcg_all):.4f}  nDCG_recoverable={np.mean(ndcg_rec):.4f}")
            print(f"[SR_DIAG] rank_mean={np.mean(ranks_rec):.1f}  rank_median={np.median(ranks_rec):.1f}  "
                  f"Hit@1={np.mean(hit1_rec):.3f}  Hit@5={np.mean(hit5_rec):.3f}  Hit@10={np.mean(hit10_rec):.3f}")
...
def train(params):
    ...
    if params.cosine_lr:
        ...
        print(f"[LR] CosineAnnealingLR: {params.train_lr} → {params.train_min_lr} over {params.epoch} epochs", flush=True)
    else:
        scheduler = None

...
def config():
    ...
    parser.add_argument('--lambda_align',      type=float, default=0.1,
                        help='Poids de la loss alignement InfoNCE utf↔urf + itf↔irf')
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_grca_tag}_logdeg_alignloss{_run_tag}.pt'
    )
...
class SentenceRetrival(nn.Module):
    ...
    def get_ranking_scores(self, graph, utf, itf, urf, irf, topk=5):
        # eval : topic seul — stable car utf/itf ne dérivent pas sous BPR
        graph.nodes['user'].data['tf'] = utf
        graph.nodes['item'].data['tf'] = itf
...
class Net(nn.Module):
    ...
    def calc_loss(self,
        ...
        return bpr_loss, ed_mi, ranking_loss, urf, irf, utf, itf
...
def train(params):
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            r_loss, mi_score, ranking_loss, urf, irf, utf, itf = net.calc_loss(
            ...
            # Alignement cosine direction seule : utf↔urf (user) + itf↔irf (item)
            # 1 - cosine_sim ∈ [0, 2], pénalise l'écart de direction, invariant aux normes
            _align_u = 1 - F.cosine_similarity(utf, urf, dim=-1).mean()
            _align_i = 1 - F.cosine_similarity(itf, irf, dim=-1).mean()
            align_loss = params.lambda_align * (_align_u + _align_i) / 2

            loss = r_loss + 0.1 * modal_loss + f_loss + mi_term + grca_loss + align_loss

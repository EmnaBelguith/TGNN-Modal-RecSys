...
def test(params, net_class=None):
    ...
    _dataset_to_modal = {'Musical_HADSF': 'musical', 'Musical_HADSF_temporal': 'musical',
                         'Baby_HADSF': 'baby', 'Baby_HADSF_temporal': 'baby',
                         'CDs_HADSF': 'cds', 'CDs_HADSF_temporal': 'cds'}
    ...
    _cat    = params.dataset_name.replace('_HADSF_temporal', '').replace('_HADSF', '')
    _df_deg = pd.read_csv(f'/home/infres/belguith/PFE/processed/{_cat}_interactions_reviews_temporal.csv')
    ...
    net.item_degree_groups = (
        ...
        set(),                         # medium  (non utilisé en temporal)

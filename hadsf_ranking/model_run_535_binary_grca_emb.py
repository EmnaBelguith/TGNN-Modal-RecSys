...
def config():
    ...
    parser.add_argument('--lambda_modal',      type=float, default=0.1,
                        help='Poids de la modal_loss InfoNCE(img, txt) dans la loss totale (défaut 0.1)')
    ...
    args.model_save_path = (
        ...
        f'{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nrev_tag}{_grca_tag}{_rinit_tag}{_bprw_tag}{_nml_tag}_logdeg_normfuse_topicmi_slowsent01_gpool_binary_grca_emb{_run_tag}.pt'
    )
...
def train(params):
    ...
    print("[INIT] item_degree_tensor injected (graph['interacts'] train edges, IDs 0..4885)", flush=True)
    ...
    for iter_idx in range(1, params.epoch):
        ...
        for rating_input_nodes, pos_graph, _neg_graph, rating_blocks in pbar:
            ...
            loss = r_loss + params.lambda_modal * modal_loss + f_loss + mi_term + grca_loss + reg_loss

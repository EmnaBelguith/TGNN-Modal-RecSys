...
def config():
 ...
 #  args.dataset_path = '/home/zheng/RatingTopicGraph/filtered_Industrial_and_Scientific_aspect_raw_10-50/filtered_Industrial_and_Scientific_aspect_raw_10-50.jsonl' # 修改为您的 JSONL 文件路径
    ...
    _DATASET_MODAL = {
        'Musical_HADSF': ('musical', 'Musical_interactions_reviews.csv'),
        'CDs_HADSF':     ('cds',     'CDs_interactions_reviews.csv'),
        'Baby_HADSF':    ('baby',    'Baby_interactions_reviews.csv'),
    }
    _bm3_subdir, _inter_csv = _DATASET_MODAL.get(args.dataset_name, ('musical', 'Musical_interactions_reviews.csv'))
    args.modal_feat_dir   = f'/home/infres/belguith/PFE/bm3_data/{_bm3_subdir}'
    args.interactions_csv = f'/home/infres/belguith/PFE/processed/{_inter_csv}'
    args.gcn_dropout = 0.8
    args.ed_alpha = 2.0
    # args.epoch = 800
    # args.num_layers = 2
    ...
    _nrev_tag  = '_norev'    if args.no_review       else ''
    _run_tag   = f'_{args.run_tag}' if args.run_tag else ''
    args.model_save_path = f'model_save/{args.dataset_name}/{args.model_short_name}_layers_{args.num_layers}_seed{args.seed}{_l2_tag}{_bs_tag}{_mi_tag}{_dp_tag}{_neg_tag}{_nneg_tag}{_warm_tag}{_nmod_tag}{_nimg_tag}{_ntxt_tag}{_alpha_tag}{_nrev_tag}{_run_tag}.pt'
...
class GCMCGraphConv(nn.Module, ABC):
    ...
    def __init__(self, \
        ...
        self.prob_score = nn.Linear(128, 1, bias=False) if not no_review else None
    ...
    def forward(self, graph, feat):
        ...
        with graph.local_scope():
            ...
            if self.no_review:
                # ablation totale reviews : attention uniforme, pas de features review
...
class MultiLayerHeteroGraphConv(nn.Module):
    ...
    def __init__(self, rating_values, review_embedding, user_size, item_size, msg_units, num_layers, aggregate='sum', dropout_rate=0.0, no_review=False):
        ...
        for l in range(num_layers):
            ...
            for rating in rating_values:
                ...
                sub_conv[rating] = GCMCGraphConv(msg_units, \
						 ...
						 add_review = (not no_review) and (l == (num_layers - 1)), \
...
class Net(nn.Module):
    ...
    def calc_loss(self,
        ...
        # k in-batch negatives : k permutations indépendantes, loss = moyenne
...
def train(params):
   ...
   # === MODAL ENCODER ===
    v_feat, t_feat = load_modal_features(params.modal_feat_dir)
...
# =====================
    ...
    import pandas as pd
    _df_deg_train = pd.read_csv(params.interactions_csv)
...
def test(params):
    """Évaluation correcte — utilise eval_abl_no_review (Net avec no_review=True)."""
    from eval_abl_no_review import test as _eval_test
    _eval_test(params)
